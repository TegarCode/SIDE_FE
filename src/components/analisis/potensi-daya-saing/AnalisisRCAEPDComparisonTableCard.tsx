...
const comparisonColumns: ComparisonColumn[] = [
  ...
  { key: "xModelAsal", label: "X Model Asal", sourceKey: "X Model Asal" },
  {
    key: "xModelTujuan",
    label: "X Model Tujuan",
    sourceKey: "X Model Tujuan"
  },
  { key: "strategy", label: "Strategy", sourceKey: "Strategy" },
  {
    key: "valueA",
    label: "Value A",
...
function getColumnValue(
  row: AnalisisRcaEpdComparisonRow,
  column: ComparisonColumn
) {
  return row[column.sourceKey];
}

export function AnalisisRCAEPDComparisonTableCard({
  title,
  rows,
  loading = false,
  errorMessage,
      ...
      columns: [
        {
          key: "no",
          label: "No",
          selector: (_row: AnalisisRcaEpdComparisonRow, index: number) =>
            index + 1,
          numeric: true
        },
        ...columns.map((column) => ({
          key: column.key,
          label: column.label,
          selector: (row: AnalisisRcaEpdComparisonRow) =>
          ...
          <tbody>
            {displayedRows.length ? (
              ...
              <tr>
                ...
                >
                  Data Country Comparison RCA & EPD belum tersedia.
                </td>
              </tr>
            )}
          </tbody>
