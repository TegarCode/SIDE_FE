...
import { DataGeneratorInvestmentDetailModal } from "@/components/data-generator/DataGeneratorInvestmentDetailModal";
...
type InvestmentResponse = {
  data?: {
    investasi_asal_ke_tujuan?: Record<
      string,
      { total?: string; per_negara?: InvestmentCellDetail[] }
    >;
...
export function DataGeneratorInvestmentTableSection({
  data,
  loading,
  originTooltipItems,
    ...
    () =>
      years.map((year) => ({
        ...
        asalKeTujuan: asNumber(
          data?.data?.investasi_asal_ke_tujuan?.[String(year)]?.total
        ),
        asalKeDunia: asNumber(
          data?.data?.investasi_asal_ke_dunia?.[String(year)]?.total
        ),
        duniaKeTujuan: asNumber(
          data?.data?.investasi_dunia_ke_tujuan?.[String(year)]?.total
        )
      })),
    [
      data?.data?.investasi_asal_ke_dunia,
      data?.data?.investasi_asal_ke_tujuan,
      data?.data?.investasi_dunia_ke_tujuan,
      years
    ]
  );
  ...
  const exportSubtitle = `Tahun ${years[0] ?? "-"}-${years[years.length - 1] ?? "-"} | Unit: ${unit} | Asal: ${originFullLabel} | Tujuan: ${destinationFullLabel} | Sumber: ${source}`;
  ...
  const detailRows = React.useMemo(() => {
    ...
    if (detailState.segment === "asal_ke_tujuan") {
      return (
        data?.data?.investasi_asal_ke_tujuan?.[String(detailState.year)]
          ?.per_negara ?? []
      );
    }
    if (detailState.segment === "asal_ke_dunia") {
      return (
        data?.data?.investasi_asal_ke_dunia?.[String(detailState.year)]
          ?.per_negara ?? []
      );
    }
    return (
      data?.data?.investasi_dunia_ke_tujuan?.[String(detailState.year)]
        ?.per_negara ?? []
    );
  }, [
    data?.data?.investasi_asal_ke_dunia,
  ...
  const detailTitle = React.useMemo(() => {
    if (!detailState.segment || detailState.year == null) return "";
    if (detailState.segment === "asal_ke_tujuan") {
      return `${investmentTypeText} ${formatCompact(originValues)} ke ${formatCompact(destinationValues)}`;
    }
    if (detailState.segment === "asal_ke_dunia") {
      return `${investmentTypeText} ${formatCompact(originValues)} ke Dunia`;
    }
    return `${investmentTypeText} Dunia ke ${formatCompact(destinationValues)}`;
  }, [
    destinationValues,
    detailState.segment,
    detailState.year,
  ...
  const detailSubtitle = React.useMemo(() => {
    ...
    return `Tahun ${detailState.year} | Unit: ${unit}`;
  }, [detailState.year, unit]);
  ...
  const handleDownload = React.useCallback(() => {
    void downloadTableAsExcel({
      filename: `data_generator_investasi_${investmentTypeText.toLowerCase()}_table`,
      title: `Data ${investmentTypeText} Investasi`,
      ...
      columns: [
        ...
        {
          key: "asal_ke_tujuan",
          label: `${investmentTypeText} ${formatCompact(originValues)} ke ${formatCompact(destinationValues)}`
        },
        {
          key: "asal_ke_dunia",
          label: `${investmentTypeText} ${formatCompact(originValues)} ke Dunia`
        },
        {
          key: "dunia_ke_tujuan",
          label: `${investmentTypeText} Dunia ke ${formatCompact(destinationValues)}`
        }
      ],
      rows: rows.map((row) => ({
  ...
  if (!data || rows.length === 0) {
    return (
      <EmptyStatePanel
        title="Data investasi belum tersedia"
        description="Atur filter investasi lalu pilih Tampilan Tabel untuk memuat hasil generator."
      />
    );
  }
          ...
          <table className="min-w-full border-separate border-spacing-0 text-sm text-slate-700">
            <thead className="bg-[#1d4ed8] text-white">
              <tr>
                ...
                <th className="border-b border-r border-blue-400 px-4 py-3 text-center font-semibold">
                  {investmentTypeText} {formatCompact(originValues)} ke{" "}
                  {formatCompact(destinationValues)}
                </th>
                <th className="border-b border-r border-blue-400 px-4 py-3 text-center font-semibold">
                  {investmentTypeText} {formatCompact(originValues)} ke Dunia
                </th>
                <th className="border-b border-blue-400 px-4 py-3 text-center font-semibold">
                  {investmentTypeText} Dunia ke{" "}
                  {formatCompact(destinationValues)}
                </th>
              </tr>
            </thead>
  ...
  return (
    ...
    >
      <>
        {content}
        <DataGeneratorInvestmentDetailModal
          ...
          source={`Sumber: ${source}`}
          exportFilename={`data_generator_investment_detail_${detailState.segment ?? "detail"}`}
        />
      </>
    </ExpandableCard>
  );
