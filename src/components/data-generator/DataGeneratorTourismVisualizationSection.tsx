...
export function DataGeneratorTourismVisualizationSection({
  data,
  loading
}: Props) {
  ...
  const topPairs = React.useMemo(() => {
    ...
    years.forEach((year, yearIndex) => {
      const perCountry = toEntryArray(
        data?.data?.pariwisata_asal_ke_tujuan?.[String(year)]?.per_negara
      );
    ...
    return [...pairMap.values()]
      ...
      .slice(0, 5);
  }, [data?.data?.pariwisata_asal_ke_tujuan, years]);
  ...
  const latestAvailableTreemapYear = React.useMemo(() => {
    ...
    return (
      availableYears.find((year) => {
        const bucket = data?.data?.pariwisata_asal_ke_tujuan?.[String(year)];
  ...
  const treemapTotalLatest = React.useMemo(
    () =>
      latestAvailableTreemapYear != null
        ...
        : 0,
    [data?.data?.pariwisata_asal_ke_tujuan, latestAvailableTreemapYear]
  );

  const subtitle = (
    <>
      {`Tahun ${years[0] ?? "-"}-${latestYear ?? "-"} | Jenis Data: ${typeDataText} | Asal: `}
  ...
  const treemapSubtitle = (
    <>
      {`Tahun ${formatYearRange(previousAvailableTreemapYear, latestAvailableTreemapYear)} | Jenis Data: ${typeDataText} | Asal: `}
      <PartySummary label="Asal" items={originItems} />
      {" | Tujuan: "}
      <PartySummary label="Tujuan" items={destinationItems} />
    </>
  ...
  const exportSubtitle = `Tahun ${years[0] ?? "-"}-${latestYear ?? "-"} | Jenis Data: ${typeDataText} | Asal: ${
    originItems.length ? originItems.join(", ") : "-"
  } | Tujuan: ${destinationItems.length ? destinationItems.join(", ") : "-"}`;
  const exportTreemapSubtitle = `Tahun ${formatYearRange(previousAvailableTreemapYear, latestAvailableTreemapYear)} | Jenis Data: ${typeDataText} | Asal: ${
    originItems.length ? originItems.join(", ") : "-"
  } | Tujuan: ${destinationItems.length ? destinationItems.join(", ") : "-"}`;
  const barChartTitle = `Bar Chart Top 5 Negara Tujuan ${typeDataText}`;
  const lineChartTitle = `Line Chart Tren Top 5 Pasangan Negara/Entitas ${typeDataText}`;
  const stackedChartTitle = `Stacked Bar Chart Perbandingan ${typeDataText} dari Negara Asal ke Tujuan dan ke Dunia`;
  const treemapChartTitle = `Treemap Pangsa Top 5 Negara Asal ${typeDataText}`;
  ...
  const stackedSeries = React.useMemo(() => {
    ...
    const categoryRows = years.flatMap((year) => {
      ...
      const tujuanEntries = toEntryArray(
        data?.data?.pariwisata_asal_ke_tujuan?.[String(year)]?.per_negara
      );
      ...
      const duniaEntries = toEntryArray(
        data?.data?.pariwisata_asal_ke_dunia?.[String(year)]?.per_negara
      );
    ...
    ({
      ...
      if (seriesName === "Asal ke Dunia") {
        const previousEntries = toEntryArray(
          data?.data?.pariwisata_asal_ke_dunia?.[String(previousYear)]
            ?.per_negara
        );
      ...
      const previousEntries = toEntryArray(
        data?.data?.pariwisata_asal_ke_tujuan?.[String(previousYear)]
          ?.per_negara
      );
  ...
  if (!data || years.length === 0 || topPairs.length === 0) {
    return (
      <EmptyStatePanel
        title="Visualisasi pariwisata belum tersedia"
        description="Atur filter pariwisata lalu pilih Tampilan Visualisasi untuk memuat chart."
      />
    );
  }
      ...
      <VisualizationCard
        ...
        expandedContent={
          <CountryGroupedBarChart
            ...
            height={560}
            unit={unit}
            filename="data_generator_tourism_bar_pairs"
      ...
      <VisualizationCard
        ...
        expandedContent={
          <PairLineChart
            ...
            height={560}
            unit={unit}
            filename="data_generator_tourism_line_pairs"
      ...
      <VisualizationCard
        ...
        expandedContent={
          <PairStackedBarChart
            ...
            height={560}
            unit={unit}
            filename="data_generator_tourism_stacked_pairs"
      ...
      <VisualizationCard
        ...
        expandedContent={
          <EntityTreemapChart
            ...
            unitLabel={unit}
            height={560}
            filename="data_generator_tourism_treemap_pairs"
