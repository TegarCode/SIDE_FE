...
type DataGeneratorTradeDetailModalProps = {
  open: boolean;
  onClose: () => void;
  title: string;
  subtitle?: string;
  rows: TradeCellDetail[];
  showOrigin?: boolean;
...
export function DataGeneratorTradeDetailModal({
  open,
  onClose,
  title,
  subtitle,
  ...
  source,
  exportFilename
}: DataGeneratorTradeDetailModalProps) {
  ...
  const toSortableNumber = React.useCallback((value: string) => {
  ...
  const handleDownload = React.useCallback(() => {
    const exportRows = [...filteredRows]
      .sort(
        (left, right) =>
          toSortableNumber(right.total) - toSortableNumber(left.total)
      )
      .map((row, index) => ({
        No: index + 1,
        ...(showOrigin ? { "Negara / Entitas Asal": row.asal } : {}),
        ...(showDestination ? { "Negara / Entitas Tujuan": row.tujuan } : {}),
        "Total Nilai": row.total
      }));
