...
import {
  ...
  useDataGeneratorTourismCountriesQuery,
  useDataGeneratorTourismCountryGroupsQuery,
  useDataGeneratorTourismSourcesQuery,
  useDataGeneratorTourismYearsQuery
} from "@/hooks/data-generator/useDataGeneratorTourismMasterQuery";
import { isUnauthorizedApiError } from "@/utils/apiError";
import { validateDataGeneratorTourismFilters } from "@/validators/dataGeneratorTourismFilters";

export type DataGeneratorTourismFilterValue = {
  origins: string[];
...
type Props = {
  value: DataGeneratorTourismFilterValue;
  onChange: (next: DataGeneratorTourismFilterValue) => void;
  onApply?: (next: DataGeneratorTourismFilterValue) => void;
  onUnauthorizedChange?: (hasUnauthorizedError: boolean) => void;
  badge?: string;
  isSubmitting?: boolean;
};

const TOURISM_DATA_TYPE_OPTIONS: SelectOption[] = [
...
export function DataGeneratorTourismFiltersPanel({
  value,
  onChange,
  onApply,
  ...
  const countriesQuery = useDataGeneratorTourismCountriesQuery();
  const countryGroupsQuery = useDataGeneratorTourismCountryGroupsQuery();
  const yearsQuery = useDataGeneratorTourismYearsQuery();
  const sourcesQuery = useDataGeneratorTourismSourcesQuery();
  ...
  const originGroupCountriesQuery =
    useDataGeneratorTourismCountriesByGroupQuery(value.originGroup);
  const destinationGroupCountriesQuery =
    useDataGeneratorTourismCountriesByGroupQuery(value.destinationGroup);
  const validationErrors = React.useMemo(
    () => validateDataGeneratorTourismFilters(value),
    [value]
  );
  ...
  const updateValue = React.useCallback(
    (patch: Partial<DataGeneratorTourismFilterValue>) => {
  ...
  React.useEffect(() => {
    ...
    const patch: Partial<DataGeneratorTourismFilterValue> = {};
  ...
  React.useEffect(() => {
    ...
    if (sourceOptions[0]?.value)
      updateValue({ source: sourceOptions[0].value });
  }, [sourceOptions, updateValue, value.source]);

  React.useEffect(() => {
    if (value.typeData) return;
    updateValue({ typeData: "Jumlah_Wisatawan" });
  }, [updateValue, value.typeData]);
  ...
  const submitWithMode = React.useCallback(
    (outputType: "table" | "chart") => {
      ...
      const errors = validateDataGeneratorTourismFilters(nextValue);
      const firstError = Object.values(errors)[0];

      if (firstError) {
        toast({
          title: "Filter pariwisata belum lengkap",
  ...
  return (
    <Accordion
      title="Filter Data Generator Pariwisata"
      description="Susun kombinasi negara, grup, rentang tahun, jenis data, dan sumber data untuk menyiapkan tampilan tabel maupun visualisasi."
      badge={badge}
      summary={
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <div className="space-y-1">
            ...
            <Select
              ...
              options={TOURISM_DATA_TYPE_OPTIONS}
              onChange={(next) => updateValue({ typeData: next || null })}
              placeholder="Pilih jenis data"
              size="sm"
              error={fieldErrors.typeData}
            />
          </div>
          <div className="space-y-1">
            <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-slate-500">
