...
function parseTopProdukInsights(
  ...
  return list
    .filter(isRecord)
    .map((item) => {
      const nilai = asNumberSeries(
        item.nilai ?? item.import ?? item.impor ?? item.export
      );
      return {
        hs:
        ...
        export: asNumberSeries(item.import ?? item.impor ?? item.export),
        exportReverse: asNumberSeries(
          item.import_reverse ?? item.export_reverse
        ),
        tujuanEkspor: parseCountryValueList(
          item.tujuan_impor ?? item.tujuan_ekspor
        ),
  ...
  const hs =
    typeof candidate.kodeHS === "string" || typeof candidate.kodeHS === "number"
      ...
      : typeof candidate.hs === "string" || typeof candidate.hs === "number"
        ? String(candidate.hs)
        : "-";
  const tujuanEkspor = parseCountryValueList(
    candidate.tujuan_impor ?? candidate.tujuan_ekspor
  );
  const inferredValue =
    fallbackYear != null
...
export function TotalImporTab({
  ...
  const timeSeries = React.useMemo(
    () => [{ label: "Nilai Impor", values: totals }],
    [totals]
  );
  ...
  const handleSearchCompetition = React.useCallback(async () => {
    ...
    try {
      ...
      if (!nextProducts.length) {
        setCompetitionError(
          "Data insight negara asal impor tidak ditemukan untuk HS terpilih."
        );
      }
    } catch {
      ...
      setHasAutoRequestedCompetition(true);
      setCompetitionError("Gagal memuat data insight negara asal impor.");
    }
  }, [
  ...
  React.useEffect(() => {
    if (competitionInsightMutation.isPending) {
      ...
      loadingCompetitionToastIdRef.current = toast({
        ...
        description: "Peta persaingan produk impor sedang diproses...",
        tone: "loading",
        durationMs: null
      });
      return;
  ...
  return (
    <div className="space-y-4">
      <Modal
        ...
        title="Peta Persaingan Produk Impor INDONESIA"
        subtitle={
          modalProduct
      ...
      >
        <TradeCompetitionInsight
          ...
          listYearLabel={latestYear}
          topDestinationLabel="Top Asal"
          unitLabel={unitLabel}
          sourceLabel={sourceLabel}
          competitorSourceLabel={competitorSourceLabel}
          emptyMessage="Detail negara asal impor belum tersedia."
          titlePrefixPrimary="Negara Asal Impor Indonesia"
          titlePrefixGlobal="Asal Impor Utama dari"
          titlePrefixAsean="Asal Impor ASEAN dari"
          showHeader={false}
      ...
      {showLoadingLayout ? (
        <>
          ...
          <section className="space-y-4">
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
              <h3 className="font-semibold tracking-tight text-slate-900">
                Peta Persaingan Produk Impor Indonesia
              </h3>
              <p className="text-xs text-slate-500">
        ...
        <FilterFallbackCard
          title="Bagian lanjutan belum tersedia"
          body="Data total impor belum tersedia untuk filter aktif."
        />
      ) : (
        <>
          <section className="space-y-4">
            ...
            <div className="grid items-stretch gap-4 xl:grid-cols-[1.8fr_1fr]">
              <ExpandableCard
                title="Peta Nilai Impor Indonesia"
                ...
                expandedContent={
                  <MapHeatLayer
                    key={`total-impor-map-expanded-${periodLabel}`}
              ...
              >
                <div className="flex h-full flex-col">
                  <MapHeatLayer
                    key={`total-impor-map-${periodLabel}`}
              ...
              <ExpandableCard
                title="Top Mitra Nilai Impor Indonesia"
                ...
                expandedContent={
                  <TopMitraTable
                    ...
                    downloadTitle="Top Mitra Nilai Impor Indonesia"
                    downloadFilename={`Top_Mitra_Nilai_Impor_Indonesia_${periodLabel.replace(/\s+/g, "_")}`}
                    downloadSource={sourceLabel ?? undefined}
                    emptyMessage="Data top mitra impor belum tersedia."
                  />
                }
              >
                <div className="flex h-full flex-col">
          ...
          <section className="space-y-4">
            <div className="grid gap-4 lg:grid-cols-[1.8fr_1fr]">
              <ExpandableCard
                title="Time Series Nilai Impor Indonesia"
                subtitle={`${periodLabel} | Unit: ${unitLabel}`}
                className="min-w-0 min-h-144"
                modalSize="full"
                expandedContent={
                  <MultiLineTrendChart
                    key={`total-impor-ts-expanded-${periodLabel}-${yearsAsc.join("-")}`}
                    series={timeSeries}
                    years={yearsAsc}
                    unit={unitLabel}
              ...
              >
                <div className="h-130">
                  <MultiLineTrendChart
                    key={`total-impor-ts-${periodLabel}-${yearsAsc.join("-")}`}
                    series={timeSeries}
                    years={yearsAsc}
                    unit={unitLabel}
                    extrasByYear={extrasByYear}
              ...
              <ExpandableCard
                title="Top Produk Nilai Impor Indonesia"
                ...
                expandedContent={
                  <TopProdukTable
                    ...
                    downloadTitle="Top Produk Nilai Impor Indonesia"
                    downloadFilename={`Top_Produk_Nilai_Impor_Indonesia_${periodLabel.replace(/\s+/g, "_")}`}
            ...
            <ExpandableCard
              ...
              expandedContent={
                <TradeCompetitionInsight
                  ...
                  emptyMessage="Data persaingan produk impor belum tersedia."
                  titlePrefixPrimary="Negara Asal Impor Indonesia"
                  titlePrefixGlobal="Asal Impor Utama dari"
                  titlePrefixAsean="Asal Impor ASEAN dari"
                  showHeader={false}
