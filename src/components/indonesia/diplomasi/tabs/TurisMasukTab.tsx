...
function parseItems(raw: unknown) {
  ...
  return list
    ...
    .map((item) => ({
      ...
      jumlahWisatawan: asNumberSeries(
        item.Jumlah_Wisatawan ?? item.jumlah_wisatawan
      ),
      share: asNumberSeries(item.share)
    }))
    .filter((item) => Object.keys(item.jumlahWisatawan).length > 0);
}
...
function parseTrendRows(
  ...
  if (!isRecord(raw)) return [];
  const data = isRecord(raw.data) ? raw.data : null;
  const trendRoot = isRecord(data?.tren_wisatawan_masuk)
    ? data.tren_wisatawan_masuk
    : isRecord(raw.tren_wisatawan_masuk)
      ? raw.tren_wisatawan_masuk
      : null;
  ...
  return items.map((item) => {
    const nilaiCurr = item.jumlahWisatawan[latestYear] ?? 0;
    const nilaiPrev =
      prevYear != null ? (item.jumlahWisatawan[prevYear] ?? 0) : null;
    return {
      alpha2: item.alpha2,
      alpha3: item.alpha3,
...
function remapWisatawanRaw(raw: unknown): unknown {
  ...
  const remapItems = (value: unknown) => {
    ...
    return value.map((entry) => {
      ...
      const jumlah = isRecord(entry.Jumlah_Wisatawan)
        ? entry.Jumlah_Wisatawan
        : isRecord(entry.jumlah_wisatawan)
          ? entry.jumlah_wisatawan
          : null;
      return {
        ...entry,
        nilai_perdagangan: jumlah ?? entry.nilai_perdagangan,
        nilai: jumlah ?? entry.nilai,
...
function TrendWisatawanTable({
  ...
  const handleDownload = React.useCallback(() => {
    downloadTableAsExcel({
      ...
      rows: sortedRows,
      filename: `Tren_Wisatawan_Masuk_${activeYear ?? "-"}`,
      sheetName: "Tren"
    });
  ...
  if (!rows.length) {
    return (
      <div className="flex h-full items-center rounded-lg border border-dashed border-slate-300 bg-slate-50 p-3 text-xs text-slate-600">
        Data tren wisatawan masuk belum tersedia.
      </div>
    );
  }
...
export function TurisMasukTab({
  ...
  const remappedRaw = React.useMemo(() => remapWisatawanRaw(raw), [raw]);
  ...
  const yearsAsc = React.useMemo(() => {
    ...
    for (const item of items) {
      for (const year of Object.keys(item.jumlahWisatawan)) {
  ...
  const totals = React.useMemo(() => {
    ...
    return yearsAsc.map((year) => {
      ...
      return items.reduce(
        (sum, item) => sum + (item.jumlahWisatawan[year] ?? 0),
        0
      );
    });
  }, [items, meta, yearsAsc]);
  ...
  const timeSeries = React.useMemo(
    () => [{ label: "Nilai Wisatawan Masuk", values: totals }],
    [totals]
  );
  ...
  return (
    <div className="space-y-4">
      {showLoadingLayout ? (
        ...
        <FilterFallbackCard
          title="Bagian lanjutan belum tersedia"
          body="Data wisatawan masuk belum tersedia untuk filter aktif."
        />
      ) : (
        <>
          <section className="space-y-4">
            ...
            <div className="grid items-stretch gap-4 xl:grid-cols-[1.8fr_1fr]">
              <ExpandableCard
                title="Peta Nilai Wisatawan Masuk"
                ...
                expandedContent={
                  <MapHeatLayer
                    key={`wisatawan-masuk-map-expanded-${periodLabel}`}
              ...
              >
                <div className="flex h-full flex-col">
                  <MapHeatLayer
                    key={`wisatawan-masuk-map-${periodLabel}`}
              ...
              <ExpandableCard
                title="Top Mitra Wisatawan Masuk"
                ...
                expandedContent={
                  <TopMitraTable
                    ...
                    downloadTitle="Top Mitra Wisatawan Masuk"
                    downloadFilename={`Top_Mitra_Wisatawan_Masuk_${periodLabel.replace(/\s+/g, "_")}`}
                    downloadSource={sourceLabel ?? undefined}
                    emptyMessage="Data top mitra wisatawan masuk belum tersedia."
                    valueLabel="Nilai Wisatawan"
                    shareLabel="Pangsa Wisatawan"
                    shareContextLabel="dari total wisatawan dunia"
                    totalLabel="Total wisatawan dunia"
                    changeLabel="Perubahan Wisatawan YoY"
          ...
          <section className="space-y-4">
            <div className="grid gap-4 lg:grid-cols-[1.8fr_1fr]">
              <ExpandableCard
                title="Time Series Nilai Wisatawan Masuk"
                subtitle={`${periodLabel} | Unit: ${unitLabel}`}
                className="min-w-0 min-h-144"
                modalSize="full"
                expandedContent={
                  <MultiLineTrendChart
                    key={`wisatawan-masuk-ts-expanded-${periodLabel}-${yearsAsc.join("-")}`}
                    series={timeSeries}
                    years={yearsAsc}
                    unit={unitLabel}
              ...
              >
                <div className="h-130">
                  <MultiLineTrendChart
                    key={`wisatawan-masuk-ts-${periodLabel}-${yearsAsc.join("-")}`}
                    series={timeSeries}
                    years={yearsAsc}
                    unit={unitLabel}
                    extrasByYear={extrasByYear}
                  ...
                  <IconTooltip label="Unduh Excel">
                    <span>
                      <Button
                        ...
                        className="inline-flex h-8 w-8 items-center justify-center rounded-md border border-slate-200 bg-white text-slate-600 transition hover:bg-slate-50 hover:text-slate-900 disabled:cursor-not-allowed disabled:opacity-60 disabled:hover:bg-white disabled:hover:text-slate-600"
                        aria-label="Unduh Excel Tren Wisatawan"
                      >
                        <ArrowDownTrayIcon className="h-4 w-4" />
                      </Button>
                    </span>
