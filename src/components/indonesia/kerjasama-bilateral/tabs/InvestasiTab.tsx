...
import {
  ...
  getSegmentSource,
  getSegmentUnit,
  type InvestmentTrendRow
} from "@/components/indonesia/kerjasama-bilateral/tabs/helpers/InvestasiTab.helpers";

type InvestasiTabProps = {
  overview: BilateralOverviewData | null;
  loading: boolean;
  error: string | null;
};

function InvestmentTrendTable({
  title,
  rows,
  sourceLabel,
  unitLabel,
    ...
    <ExpandableCard
      ...
      expandedContent={
        <TopMitraTable
          ...
          emptyMessage="Data tren investasi belum tersedia."
          valueLabel="Nilai Investasi"
          shareLabel="Pangsa Investasi"
          shareContextLabel="dari total investasi"
...
export function InvestasiTab({ overview, loading, error }: InvestasiTabProps) {
  ...
  const chartSeries = React.useMemo(
    () => [
      { label: "Investasi Masuk", values: inboundTotals },
      { label: "Investasi Keluar", values: outboundTotals }
    ],
  ...
  return (
    <div className="space-y-4">
      {loading ? (
        <>
          ...
          <section className="grid gap-4 xl:grid-cols-2">
            {Array.from({ length: 4 }).map((_, index) => (
              <div
                key={`investment-table-skeleton-${index}`}
        ...
        <FilterFallbackCard
          title="Data investasi bilateral gagal dimuat"
          body={error}
        />
      ) : shouldShowEmptyFallback ? (
        <FilterFallbackCard
          title="Data investasi bilateral belum tersedia"
          body="Data inbound maupun outbound investasi pada mitra tujuan aktif belum tersedia."
        />
      ) : (
        <>
          <ExpandableCard
            title="Tren Investasi Masuk & Keluar Indonesia ke Mitra Tujuan"
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                ...
                <p className="mt-1 text-xl font-semibold tracking-tight text-slate-900">
                  {formatInvestmentNumber(inboundAllYearsTotal)}
                </p>
              ...
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                ...
                <p className="mt-1 text-xl font-semibold tracking-tight text-slate-900">
                  {formatInvestmentNumber(outboundAllYearsTotal)}
                </p>
                <p className="text-[11px] text-slate-500">
                  Akumulasi seluruh tahun | Unit: {unitLabel}
                </p>
          ...
          <section className="grid gap-4 xl:grid-cols-2">
            <ExpandableCard
              ...
              expandedContent={
                <TopMitraTable
                  ...
                  downloadFilename={`Top_Mitra_Investasi_Masuk_${inboundLatestYear ?? "-"}`}
            ...
            <ExpandableCard
              title="Top Mitra Investasi Keluar dari Indonesia"
              ...
              expandedContent={
                <TopMitraTable
                  ...
                  downloadTitle="Top Mitra Investasi Keluar dari Indonesia"
                  downloadFilename={`Top_Mitra_Investasi_Keluar_${outboundLatestYear ?? "-"}`}
                  downloadSource={outboundSourceLabel ?? undefined}
                  downloadNotes={outboundSelectedPartnersNote}
                  emptyMessage="Data top mitra investasi keluar belum tersedia."
                  valueLabel="Nilai Investasi"
                  shareLabel="Pangsa Investasi"
                  shareContextLabel="dari total investasi"
                  totalLabel="Total investasi"
            ...
            <InvestmentTrendTable
              title="Tren Investasi Keluar dari Indonesia"
