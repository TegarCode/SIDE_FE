...
} from "@/components/indonesia/kerjasama-bilateral/tabs/helpers/PariwisataTab.helpers";

type PariwisataTabProps = {
  overview: BilateralOverviewData | null;
  loading: boolean;
  error: string | null;
};

function TourismTrendTable({
  title,
  rows,
  sourceLabel,
  unitLabel,
    ...
    <ExpandableCard
      ...
      expandedContent={
        <TopMitraTable
          ...
          emptyMessage="Data tren belum tersedia."
          valueLabel="Nilai Wisatawan"
          shareLabel="Pangsa Wisatawan"
          shareContextLabel="dari total wisatawan"
...
export function PariwisataTab({
  ...
  const unitLabel = React.useMemo(() => getSegmentUnit(raw, "inbound"), [raw]);
  ...
  const chartSeries = React.useMemo(
    () => [
      { label: "Wisatawan Masuk", values: inboundTotals },
      { label: "Wisatawan Keluar", values: outboundTotals }
    ],
  ...
  return (
    <div className="space-y-4">
      {loading ? (
        <>
          <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <h3 className="font-semibold tracking-tight text-slate-900">
              Tren Wisatawan Masuk & Keluar Indonesia ke Mitra Tujuan
            </h3>
            <p className="text-xs text-slate-500">
              Memuat data tren wisatawan...
            </p>
            <div className="mt-4">
              <ChartSkeleton />
            </div>
          ...
          <section className="grid gap-4 xl:grid-cols-2">
            {Array.from({ length: 4 }).map((_, index) => (
              <div
                key={`tourism-table-skeleton-${index}`}
        ...
        <FilterFallbackCard
          title="Data pariwisata bilateral gagal dimuat"
          body={error}
        />
      ) : shouldShowEmptyFallback ? (
        <FilterFallbackCard
          title="Data pariwisata bilateral belum tersedia"
          body="Data inbound maupun outbound wisatawan pada mitra tujuan aktif belum tersedia."
        />
      ) : (
        <>
          <ExpandableCard
            title="Tren Wisatawan Masuk & Keluar Indonesia ke Mitra Tujuan"
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                ...
                <p className="mt-1 text-xl font-semibold tracking-tight text-slate-900">
                  {formatTourismNumber(inboundAllYearsTotal)}
                </p>
              ...
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                ...
                <p className="mt-1 text-xl font-semibold tracking-tight text-slate-900">
                  {formatTourismNumber(outboundAllYearsTotal)}
                </p>
                <p className="text-[11px] text-slate-500">
                  Akumulasi seluruh tahun | Unit: {unitLabel}
                </p>
          ...
          <section className="grid gap-4 xl:grid-cols-2">
            <ExpandableCard
              title="Top Mitra Wisatawan Masuk ke Indonesia"
              ...
              expandedContent={
                <TopMitraTable
                  ...
                  downloadTitle="Top Mitra Wisatawan Masuk ke Indonesia"
                  downloadFilename={`Top_Mitra_Wisatawan_Masuk_${inboundLatestYear ?? "-"}`}
            ...
            <ExpandableCard
              title="Top Mitra Wisatawan Keluar dari Indonesia"
              ...
              expandedContent={
                <TopMitraTable
                  ...
                  downloadTitle="Top Mitra Wisatawan Keluar dari Indonesia"
                  downloadFilename={`Top_Mitra_Wisatawan_Keluar_${outboundLatestYear ?? "-"}`}
                  downloadSource={outboundSourceLabel ?? undefined}
                  downloadNotes={outboundSelectedPartnersNote}
                  emptyMessage="Data top mitra wisatawan keluar belum tersedia."
                  valueLabel="Nilai Wisatawan"
                  shareLabel="Pangsa Wisatawan"
                  shareContextLabel="dari total wisatawan"
                  totalLabel="Total wisatawan"
            ...
            <TourismTrendTable
              title="Tren Wisatawan Masuk ke Indonesia"
            ...
            <TourismTrendTable
              title="Tren Wisatawan Keluar dari Indonesia"
