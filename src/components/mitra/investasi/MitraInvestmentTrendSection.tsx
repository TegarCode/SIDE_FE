...
import type {
  ...
  MitraMultiInvestmentTimeseriesPoint
} from "@/type/mitra";
import { formatExportCountryList } from "@/utils/chartExport";

type MitraInvestmentTrendSectionProps = {
  data: MitraMultiInvestmentData | null | undefined;
  loading: boolean;
};
...
const METRICS: MetricConfig[] = [
  {
    key: "inbound",
    title: "Tren Nilai Investasi Masuk Tahunan",
    color: "#0f766e"
  },
  {
    key: "outbound",
    title: "Tren Nilai Investasi Keluar Tahunan",
    color: "#b91c1c"
  }
];
...
function getMetricValue(
  point: MitraMultiInvestmentTimeseriesPoint | undefined,
  key: MetricKey
) {
  if (!point) return null;
  return key === "inbound" ? point.inboundValue : point.outboundValue;
}
...
function TrendCard({
  ...
  metric: MetricConfig;
  timeseries: MitraMultiInvestmentTimeseriesPoint[];
  latestPoint: MitraMultiInvestmentTimeseriesPoint | undefined;
  prevPoint: MitraMultiInvestmentTimeseriesPoint | undefined;
  unit: string;
  sourceName: string;
  ...
  const content = loading ? (
    ...
    <div className="space-y-2">
      <TradeAnnualAreaChart
        data={chartData}
        primaryLabel={metric.title.replace("Tren ", "")}
        secondaryLabel=""
        unit={unit}
        height={250}
...
export function MitraInvestmentTrendSection({
  data,
  loading
}: MitraInvestmentTrendSectionProps) {
    ...
    <section className="space-y-3">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500">
          Perbandingan tren nilai investasi masuk dan investasi keluar
          berdasarkan kombinasi asal dan tujuan yang dipilih.
        </p>
