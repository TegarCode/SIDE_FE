...
import type {
  ...
  MitraMultiServiceTimeseriesPoint
} from "@/type/mitra";
import {
...
type MitraServiceTrendSectionProps = {
...
const METRICS = [
  { key: "inbound" as MetricKey, title: "Tren Jasa Masuk", color: "#0f766e" },
  { key: "outbound" as MetricKey, title: "Tren Jasa Keluar", color: "#b91c1c" }
];
...
function getMetricValue(
  point: MitraMultiServiceTimeseriesPoint | undefined,
  key: MetricKey
) {
...
function TrendCard({
  ...
  metric: { key: MetricKey; title: string; color: string };
  timeseries: MitraMultiServiceTimeseriesPoint[];
  latestPoint: MitraMultiServiceTimeseriesPoint | undefined;
  prevPoint: MitraMultiServiceTimeseriesPoint | undefined;
  unit: string;
  sourceName: string;
  ...
  const handleRegisterDownload = React.useCallback(
    (handler: (() => void) | null) => setDownloadHandler(() => handler),
    []
  );
...
export function MitraServiceTrendSection({
  data,
  loading
}: MitraServiceTrendSectionProps) {
    ...
    <section className="space-y-3">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500">
          Perbandingan tren jasa masuk dan keluar per tahun berdasarkan filter
