...
import { useMitraOverviewTopTourismQuery } from "@/hooks/mitra/useMitraOverviewTopTourismQuery";
import type { MitraOverviewInvestmentRow } from "@/type/mitra";

type PariwisataOverviewTabProps = {
...
export function PariwisataOverviewTab({
  countryCode,
  countryName
}: PariwisataOverviewTabProps) {
  const { toast, dismiss } = useToast();
  const query = useMitraOverviewTopTourismQuery(countryCode);
  ...
  const toastKey = React.useMemo(() => {
    ...
    return `mitra-top-tourism-${normalizedCountryCode}`;
  }, [normalizedCountryCode]);
  const inboundRaw = React.useMemo(
    ...
    [data?.inbound, data?.latestYear, data?.prevYear]
  );
  const outboundRaw = React.useMemo(
    ...
    [data?.outbound, data?.latestYear, data?.prevYear]
  );
  ...
  React.useEffect(() => {
    ...
    if (query.isFetching) {
      ...
      loadingToastIdRef.current = toast({
        title: "Sedang tarik data pariwisata negara mitra",
        description: `Memuat data pariwisata untuk ${countryName}.`,
        tone: "loading",
        durationMs: null
      });
      return;
  ...
  React.useEffect(() => {
    ...
    toast({
      title: "Data pariwisata negara mitra siap",
      description: `Data pariwisata untuk ${countryName} berhasil dimuat.`,
      tone: "success"
    });
  }, [
    countryName,
  ...
  if (query.error) {
    return (
      <FilterFallbackCard
        title="Data overview pariwisata gagal dimuat"
        body={`Terjadi kesalahan saat mengambil top pariwisata untuk ${countryName}.`}
      />
    );
  }
  ...
  if (!data) {
    return (
      <FilterFallbackCard
        title="Data overview pariwisata belum tersedia"
        body={`Data pariwisata untuk ${countryName} belum tersedia.`}
      />
    );
  }
  ...
  const periodLabel = formatYearPeriod(data.latestYear, data.prevYear);

  return (
    <div className="grid gap-4 xl:grid-cols-2">
      <TourismSection
        title="Top Negara/Entitas Asal Turis ke Indonesia"
        subtitle={`Tahun ${periodLabel} | Unit: ${data.unit}`}
        ...
        tableContent={
          <TopMitraTable
            raw={inboundRaw}
            unitLabel={data.unit}
            downloadTitle={`Top Negara/Entitas Asal Turis ke Indonesia`}
            downloadFilename={`Top_Asal_Turis_${displayCountryName.replace(/\s+/g, "_")}_${data.latestYear ?? "-"}`}
        ...
        chartContent={
          <TopCountryComparisonBarChart
            ...
            unitLabel={data.unit}
            filename={`Top_Asal_Turis_${displayCountryName.replace(/\s+/g, "_")}_${data.latestYear ?? "-"}`}
      ...
      <TourismSection
        title="Top Negara/Entitas Tujuan Turis dari Indonesia"
        subtitle={`Tahun ${periodLabel} | Unit: ${data.unit}`}
        sourceLabel={data.source}
        ...
        tableContent={
          <TopMitraTable
            raw={outboundRaw}
            unitLabel={data.unit}
            downloadTitle={`Top Negara/Entitas Tujuan Turis dari Indonesia`}
            downloadFilename={`Top_Tujuan_Turis_${displayCountryName.replace(/\s+/g, "_")}_${data.latestYear ?? "-"}`}
            downloadSource={data.source ?? undefined}
            downloadNotes={selectedCountryNote}
        ...
        chartContent={
          <TopCountryComparisonBarChart
            ...
            prevYear={data.prevYear}
            unitLabel={data.unit}
            filename={`Top_Tujuan_Turis_${displayCountryName.replace(/\s+/g, "_")}_${data.latestYear ?? "-"}`}
