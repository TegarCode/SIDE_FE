...
import type {
  ...
  MitraTradeOverviewTimeseriesPoint
} from "@/type/mitra";
import { formatExportCountryList } from "@/utils/chartExport";

type MitraTradeAnnualTrendSectionProps = {
...
type MetricKey = "export" | "import" | "total" | "balance";
...
const METRIC_CONFIGS: MetricConfig[] = [
  { key: "export", title: "Tren Nilai Ekspor Tahunan", color: "#2563eb" },
  { key: "import", title: "Tren Nilai Impor Tahunan", color: "#ef4444" },
  { key: "total", title: "Tren Total Perdagangan Tahunan", color: "#f97316" },
  { key: "balance", title: "Tren Neraca Perdagangan Tahunan", color: "#7c3aed" }
];
...
function getMetricValues(
  point: MitraTradeOverviewTimeseriesPoint | undefined,
  key: MetricKey
) {
...
function buildMetricSeries(
  ...
  return timeseries.map((item) => ({
    year: item.year,
    primary: getMetricValues(item, key),
    secondary: null
  }));
}
...
function AnnualTrendCard({
  ...
  metric: MetricConfig;
  timeseries: MitraTradeOverviewTimeseriesPoint[];
  latestPoint: MitraTradeOverviewTimeseriesPoint | undefined;
  prevPoint: MitraTradeOverviewTimeseriesPoint | undefined;
  unit: string;
  sourceName: string;
  ...
  const chartData = React.useMemo(
    () => buildMetricSeries(timeseries, metric.key),
    [metric.key, timeseries]
  );
  const latestValue = getMetricValues(latestPoint, metric.key);
  const prevValue = getMetricValues(prevPoint, metric.key);
  ...
  const subtitle =
    firstYear != null && lastYear != null && firstYear !== lastYear
      ? `Tahun ${firstYear}-${lastYear} | Unit: ${unit || "-"}`
      : `Tahun ${lastYear ?? "-"} | Unit: ${unit || "-"}`;
  const primaryLabel = metric.title.replace("Tren ", "");
...
export function MitraTradeAnnualTrendSection({
  data,
  loading,
  exportOrigins,
  exportDestinations
}: MitraTradeAnnualTrendSectionProps) {
  const resolvedOrigins = React.useMemo(
    () => (data ? resolveCountryNames(data.origin, data.originNames) : []),
    [data]
  );
  const resolvedDestinations = React.useMemo(
    () =>
      data ? resolveCountryNames(data.destination, data.destinationNames) : [],
    [data]
  );
  const origins = exportOrigins?.length ? exportOrigins : resolvedOrigins;
  const destinations = exportDestinations?.length
    ? exportDestinations
  ...
  const exportSubtitle = React.useMemo(
    () =>
      `Tahun ${timeseries[0]?.year ?? "-"}${timeseries.length > 1 ? `-${timeseries[timeseries.length - 1]?.year ?? "-"}` : ""} | Unit: ${data?.unit ?? "-"} | Asal: ${formatExportCountryList(origins)} | Tujuan: ${formatExportCountryList(destinations)}`,
    [data?.unit, destinations, origins, timeseries]
  );
    ...
    <section className="space-y-3">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500">
          Perbandingan tren nilai ekspor, impor, volume, dan neraca per tahun.
        </p>
