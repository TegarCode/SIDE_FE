...
function getMetricLabel(metricMode: EnergyMetricMode) {
...
export function EnergyTradeOverviewGrid({
  metricMode,
  data,
  loading = false
}: EnergyTradeOverviewGridProps) {
  ...
  const summaryCards: DiplomasiSummaryCardView[] = [
    {
      ...
      note: `Ringkasan ekspor sektor energi Tahun ${latestYear} dibandingkan Tahun ${previousYear}.`,
      highlight: null,
      prevHighlight: null,
      highlightType: "none",
    ...
    },
    {
      ...
      year: String(latestYear),
      prevYear: String(previousYear),
      note: `Ringkasan impor sektor energi Tahun ${latestYear} dibandingkan Tahun ${previousYear}.`,
      highlight: null,
      prevHighlight: null,
      highlightType: "none",
      sourceName: data?.sourceName ?? "-"
    },
    {
      ...
      year: String(latestYear),
      prevYear: String(previousYear),
      note: `Ringkasan total perdagangan sektor energi Tahun ${latestYear} dibandingkan Tahun ${previousYear}.`,
      highlight: null,
      prevHighlight: null,
      highlightType: "none",
      sourceName: data?.sourceName ?? "-"
  ...
  const mapTitle = `Peta Sebaran Sektor Energi ${metricLabel}`;
  const sourceName = data?.sourceName ?? "-";
  const tableTitle = `Top Mitra ${metricLabel} Sektor Energi`;
  const comparisonYearsLabel =
    previousYear !== latestYear
      ? `${previousYear}-${latestYear}`
      : String(latestYear);
  const mapSubtitle = loading
    ? "Sedang mengambil data peta sektor energi..."
    : `Tahun ${comparisonYearsLabel} | Unit: ${data?.unit ?? "US$"}`;
  const tableSubtitle = loading
    ? "Sedang mengambil data top mitra sektor energi..."
  ...
  async function handleDownloadMap() {
    try {
      ...
      await downloadComposedPng({
        imageUrl,
        filename: `Peta_Sebaran_Sektor_Energi_${metricLabel.replace(/[^\w]+/g, "_")}`,
        title: mapTitle,
  ...
  const tableContent = (
    <TopMitraTable
      ...
      emptyMessage={`Data ${metricLabel.toLowerCase()} sektor energi belum tersedia untuk filter aktif.`}
      onRegisterDownload={registerTableDownload}
      onSortColumnChange={handleSortColumnChange}
      downloadTitle={tableTitle}
      downloadFilename={`Top_Mitra_Sektor_Energi_${metricLabel.replace(/[^\w]+/g, "_")}`}
