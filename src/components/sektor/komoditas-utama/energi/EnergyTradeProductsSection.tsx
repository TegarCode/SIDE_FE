...
import type { EnergyTradeProductsResult } from "@/type/komoditasUtama";
import type { EnergyMetricMode } from "./EnergyTradeHeroSection";

type EnergyTradeProductsSectionProps = {
  data: EnergyTradeProductsResult | null | undefined;
  loading: boolean;
...
function getMetricKey(metricMode: EnergyMetricMode) {
...
export function EnergyTradeProductsSection({
  data,
  loading,
  metricMode
}: EnergyTradeProductsSectionProps) {
  ...
  const cardSubtitle = loading
    ? "Sedang mengambil data produk sektor energi..."
    : viewMode === "table"
  ...
  const titleNode = (
    <span className="inline-flex items-center gap-1.5">
      <span>{`Produk ${metricLabel} Sektor Energi`}</span>
      <HoverInfoTooltip
        content={
  ...
  const tableContent = (
    <div className="space-y-2">
      <TopProdukTable
        raw={tableRaw}
        unitLabel={unitLabel}
        downloadTitle={`Produk ${metricLabel} Sektor Energi`}
        downloadFilename={`Produk_Sektor_Energi_${metricLabel.replace(/[^\w]+/g, "_")}`}
        ...
        emptyMessage={`Data produk ${metricLabel.toLowerCase()} sektor energi belum tersedia.`}
      />
  ...
  const treemapBaseProps = {
    ...
    filename: `Treemap_Produk_Sektor_Energi_${metricLabel.replace(/[^\w]+/g, "_")}`,
    exportTitle: `Treemap Produk ${metricLabel} Sektor Energi`,
    exportSubtitle,
