...
import type { HilirisasiTradeFlowResult } from "@/type/komoditasUtama";
...
type HilirisasiTradeOverviewGridProps = {
  metricMode: HilirisasiMetricMode;
  data: HilirisasiTradeFlowResult | null | undefined;
  loading?: boolean;
};
...
export function HilirisasiTradeOverviewGrid({
  metricMode,
  data,
  loading = false
}: HilirisasiTradeOverviewGridProps) {
  ...
  const tradeTotalsByYear = React.useMemo(() => {
    const output: Record<number, number> = {};
    for (const year of data?.years ?? []) {
      output[year] = (data?.items ?? []).reduce(
        (sum, item) => sum + (item.nilai_perdagangan[year] ?? 0),
        0
      );
    }
    return output;
  }, [data?.items, data?.years]);
  const balanceTotalsByYear = React.useMemo(() => {
    const output: Record<number, number> = {};
    for (const year of data?.years ?? []) {
      output[year] = (data?.items ?? []).reduce(
  ...
  const totalTradeNow = tradeTotalsByYear[latestYear] ?? 0;
  const totalTradePrev = tradeTotalsByYear[previousYear] ?? 0;
  const balanceNow = balanceTotalsByYear[latestYear] ?? 0;
  const balancePrev = balanceTotalsByYear[previousYear] ?? 0;
  ...
  const totalMetricByYear = React.useMemo(() => {
    const output: Record<number, number> = {};
    for (const year of data?.years ?? []) {
      const tradeValue = tradeTotalsByYear[year] ?? 0;
      const balanceValue = balanceTotalsByYear[year] ?? 0;
      output[year] =
    ...
    return output;
  }, [balanceTotalsByYear, data?.years, metricMode, tradeTotalsByYear]);
  const summaryCards: DiplomasiSummaryCardView[] = [
    {
      id: "hilirisasi-total-export",
      ...
      note: `Ringkasan ekspor sektor hilirisasi Tahun ${latestYear} dibandingkan Tahun ${previousYear}.`,
      highlight: null,
      prevHighlight: null,
      highlightType: "none",
    ...
    },
    {
      ...
      year: String(latestYear),
      prevYear: String(previousYear),
      note: `Ringkasan impor sektor hilirisasi Tahun ${latestYear} dibandingkan Tahun ${previousYear}.`,
      highlight: null,
      prevHighlight: null,
      highlightType: "none",
      sourceName: data?.sourceName ?? "-"
    },
    {
      ...
      year: String(latestYear),
      prevYear: String(previousYear),
      note: `Ringkasan total perdagangan sektor hilirisasi Tahun ${latestYear} dibandingkan Tahun ${previousYear}.`,
      highlight: null,
      prevHighlight: null,
      highlightType: "none",
      sourceName: data?.sourceName ?? "-"
  ...
  const mapTitle = `Peta Sebaran Sektor Hilirisasi ${metricLabel}`;
  const sourceName = data?.sourceName ?? "-";
  const tableTitle = `Top Mitra ${metricLabel} Sektor Hilirisasi`;
  const comparisonYearsLabel =
    previousYear !== latestYear
      ? `${previousYear}-${latestYear}`
      : String(latestYear);
  const mapSubtitle = loading
    ? "Sedang mengambil data peta sektor hilirisasi..."
    : `Tahun ${comparisonYearsLabel} | Unit: ${data?.unit ?? "US$"}`;
  const tableSubtitle = loading
    ? "Sedang mengambil data top mitra sektor hilirisasi..."
  ...
  async function handleDownloadMap() {
    try {
      ...
      await downloadComposedPng({
        imageUrl,
        filename: `Peta_Sebaran_Sektor_Hilirisasi_${metricLabel.replace(/[^\w]+/g, "_")}`,
        title: mapTitle,
  ...
  const tableContent = (
    <TopMitraTable
      ...
      emptyMessage={`Data ${metricLabel.toLowerCase()} sektor hilirisasi belum tersedia untuk filter aktif.`}
      onRegisterDownload={registerTableDownload}
      onSortColumnChange={handleSortColumnChange}
      downloadTitle={tableTitle}
      downloadFilename={`Top_Mitra_Sektor_Hilirisasi_${metricLabel.replace(/[^\w]+/g, "_")}`}
  ...
  return (
    <div className="mb-15 grid min-w-0 items-stretch gap-4 xl:grid-cols-12">
