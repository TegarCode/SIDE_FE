...
type HilirisasiTradeProductsSectionProps = {
...
export function HilirisasiTradeProductsSection({
  data,
  loading,
  metricMode
}: HilirisasiTradeProductsSectionProps) {
  ...
  const cardSubtitle = loading
    ? "Sedang mengambil data produk sektor hilirisasi..."
    : viewMode === "table"
  ...
  const titleNode = (
    <span className="inline-flex items-center gap-1.5">
      <span>{`Produk ${metricLabel} Sektor Hilirisasi`}</span>
      <HoverInfoTooltip
        content={
  ...
  const tableContent = (
    <div className="space-y-2">
      <TopProdukTable
        raw={tableRaw}
        unitLabel={unitLabel}
        downloadTitle={`Produk ${metricLabel} Sektor Hilirisasi`}
        downloadFilename={`Produk_Sektor_Hilirisasi_${metricLabel.replace(/[^\w]+/g, "_")}`}
        ...
        emptyMessage={`Data produk ${metricLabel.toLowerCase()} sektor hilirisasi belum tersedia.`}
      />
  ...
  const treemapBaseProps = {
    ...
    filename: `Treemap_Produk_Sektor_Hilirisasi_${metricLabel.replace(/[^\w]+/g, "_")}`,
    exportTitle: `Treemap Produk ${metricLabel} Sektor Hilirisasi`,
    exportSubtitle,
