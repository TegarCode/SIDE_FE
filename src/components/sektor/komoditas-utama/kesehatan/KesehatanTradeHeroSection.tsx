...
export type KesehatanMetricMode =
  | "total_export"
  | "total_import"
  | "total_trade";

type KesehatanTradeHeroSectionProps = {
  metricMode: KesehatanMetricMode;
  onMetricChange: (value: KesehatanMetricMode) => void;
};

export function KesehatanTradeHeroSection({
  metricMode,
  onMetricChange
}: KesehatanTradeHeroSectionProps) {
  return (
    <section className="space-y-3">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div className="space-y-1">
          <h2 className="text-lg font-semibold tracking-tight text-slate-900">
            Perdagangan Indonesia Ke Global Sektor Kesehatan
          </h2>
          <p className="max-w-3xl text-sm text-slate-600">
            Ringkasan analitik perdagangan sektor kesehatan Indonesia ke global
