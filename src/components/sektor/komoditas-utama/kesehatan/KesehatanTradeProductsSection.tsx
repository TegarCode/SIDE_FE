...
import type { KesehatanTradeProductsResult } from "@/type/komoditasUtama";
import type { KesehatanMetricMode } from "./KesehatanTradeHeroSection";

type KesehatanTradeProductsSectionProps = {
  data: KesehatanTradeProductsResult | null | undefined;
  loading: boolean;
...
function getMetricKey(metricMode: KesehatanMetricMode) {
...
export function KesehatanTradeProductsSection({
  data,
  loading,
  metricMode
}: KesehatanTradeProductsSectionProps) {
  ...
  const cardSubtitle = loading
    ? "Sedang mengambil data produk sektor kesehatan..."
    : viewMode === "table"
  ...
  const titleNode = (
    <span className="inline-flex items-center gap-1.5">
      <span>{`Produk ${metricLabel} Sektor Kesehatan`}</span>
      <HoverInfoTooltip
        content={
  ...
  const tableContent = (
    <div className="space-y-2">
      <TopProdukTable
        raw={tableRaw}
        unitLabel={unitLabel}
        downloadTitle={`Produk ${metricLabel} Sektor Kesehatan`}
        downloadFilename={`Produk_Sektor_Kesehatan_${metricLabel.replace(/[^\w]+/g, "_")}`}
        ...
        emptyMessage={`Data produk ${metricLabel.toLowerCase()} sektor kesehatan belum tersedia.`}
      />
  ...
  const treemapBaseProps = {
    ...
    filename: `Treemap_Produk_Sektor_Kesehatan_${metricLabel.replace(/[^\w]+/g, "_")}`,
    exportTitle: `Treemap Produk ${metricLabel} Sektor Kesehatan`,
    exportSubtitle,
