...
import type { MineralKritisTradeFlowResult } from "@/type/komoditasUtama";
import { downloadComposedPng } from "@/utils/downloadComposedPng";
import type { MineralKritisMetricMode } from "./MineralKritisTradeHeroSection";

type MineralKritisTradeOverviewGridProps = {
  metricMode: MineralKritisMetricMode;
  data: MineralKritisTradeFlowResult | null | undefined;
  loading?: boolean;
};
...
function getMetricLabel(metricMode: MineralKritisMetricMode) {
...
export function MineralKritisTradeOverviewGrid({
  metricMode,
  data,
  loading = false
}: MineralKritisTradeOverviewGridProps) {
  ...
  const summaryCards: DiplomasiSummaryCardView[] = [
    {
      id: "mineral-kritis-total-export",
      ...
      note: `Ringkasan ekspor sektor mineral kritis Tahun ${latestYear} dibandingkan Tahun ${previousYear}.`,
      highlight: null,
      prevHighlight: null,
      highlightType: "none",
    ...
    {
      id: "mineral-kritis-total-import",
      ...
      year: String(latestYear),
      prevYear: String(previousYear),
      note: `Ringkasan impor sektor mineral kritis Tahun ${latestYear} dibandingkan Tahun ${previousYear}.`,
      highlight: null,
      prevHighlight: null,
      highlightType: "none",
      sourceName: data?.sourceName ?? "-"
    },
    {
      ...
      year: String(latestYear),
      prevYear: String(previousYear),
      note: `Ringkasan total perdagangan sektor mineral kritis Tahun ${latestYear} dibandingkan Tahun ${previousYear}.`,
      highlight: null,
      prevHighlight: null,
      highlightType: "none",
      sourceName: data?.sourceName ?? "-"
  ...
  const mapTitle = `Peta Sebaran Sektor Mineral Kritis ${metricLabel}`;
  const sourceName = data?.sourceName ?? "-";
  const tableTitle = `Top Mitra ${metricLabel} Sektor Mineral Kritis`;
  const comparisonYearsLabel =
    previousYear !== latestYear
      ? `${previousYear}-${latestYear}`
      : String(latestYear);
  const mapSubtitle = loading
    ? "Sedang mengambil data peta sektor mineral kritis..."
    : `Tahun ${comparisonYearsLabel} | Unit: ${data?.unit ?? "US$"}`;
  const tableSubtitle = loading
    ? "Sedang mengambil data top mitra sektor mineral kritis..."
  ...
  async function handleDownloadMap() {
    try {
      ...
      await downloadComposedPng({
        imageUrl,
        filename: `Peta_Sebaran_Sektor_Mineral_Kritis_${metricLabel.replace(/[^\w]+/g, "_")}`,
        title: mapTitle,
  ...
  const tableContent = (
    <TopMitraTable
      ...
      emptyMessage={`Data ${metricLabel.toLowerCase()} sektor mineral kritis belum tersedia untuk filter aktif.`}
      onRegisterDownload={registerTableDownload}
      onSortColumnChange={handleSortColumnChange}
      downloadTitle={tableTitle}
      downloadFilename={`Top_Mitra_Sektor_Mineral_Kritis_${metricLabel.replace(/[^\w]+/g, "_")}`}
