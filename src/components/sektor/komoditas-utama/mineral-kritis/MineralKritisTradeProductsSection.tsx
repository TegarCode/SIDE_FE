...
import type { MineralKritisTradeProductsResult } from "@/type/komoditasUtama";
import type { MineralKritisMetricMode } from "./MineralKritisTradeHeroSection";

type MineralKritisTradeProductsSectionProps = {
  data: MineralKritisTradeProductsResult | null | undefined;
  loading: boolean;
...
function getMetricKey(metricMode: MineralKritisMetricMode) {
...
export function MineralKritisTradeProductsSection({
  data,
  loading,
  metricMode
}: MineralKritisTradeProductsSectionProps) {
  ...
  const cardSubtitle = loading
    ? "Sedang mengambil data produk sektor mineral kritis..."
    : viewMode === "table"
  ...
  const titleNode = (
    <span className="inline-flex items-center gap-1.5">
      <span>{`Produk ${metricLabel} Sektor Mineral Kritis`}</span>
      <HoverInfoTooltip
        content={
  ...
  const tableContent = (
    <div className="space-y-2">
      <TopProdukTable
        raw={tableRaw}
        unitLabel={unitLabel}
        downloadTitle={`Produk ${metricLabel} Sektor Mineral Kritis`}
        downloadFilename={`Produk_Sektor_Mineral_Kritis_${metricLabel.replace(/[^\w]+/g, "_")}`}
        ...
        emptyMessage={`Data produk ${metricLabel.toLowerCase()} sektor mineral kritis belum tersedia.`}
      />
  ...
  const treemapBaseProps = {
    ...
    filename: `Treemap_Produk_Sektor_Mineral_Kritis_${metricLabel.replace(/[^\w]+/g, "_")}`,
    exportTitle: `Treemap Produk ${metricLabel} Sektor Mineral Kritis`,
    exportSubtitle,
