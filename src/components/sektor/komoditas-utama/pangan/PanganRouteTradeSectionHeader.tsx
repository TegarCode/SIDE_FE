...
type PanganRouteTradeSectionHeaderProps = {
  metricMode: PanganMetricMode;
  onMetricChange: (value: PanganMetricMode) => void;
};

export function PanganRouteTradeSectionHeader({
  metricMode,
  onMetricChange
}: PanganRouteTradeSectionHeaderProps) {
  return (
    <section className="space-y-3">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div className="space-y-1">
          <h2 className="text-lg font-semibold tracking-tight text-slate-900">
            Perdagangan Rute Sektor Pangan
          </h2>
          <p className="max-w-3xl text-sm text-slate-600">
            Analitik lanjutan sektor pangan berdasarkan negara asal, negara
