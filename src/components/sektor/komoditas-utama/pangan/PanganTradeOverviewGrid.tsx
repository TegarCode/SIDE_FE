...
export function PanganTradeOverviewGrid({
  metricMode,
  data,
  loading = false
}: PanganTradeOverviewGridProps) {
  ...
  const summaryCards: DiplomasiSummaryCardView[] = [
    {
      ...
      note: `Ringkasan ekspor sektor pangan Tahun ${latestYear} dibandingkan Tahun ${previousYear}.`,
      highlight: null,
      prevHighlight: null,
      highlightType: "none",
    ...
    },
    {
      ...
      year: String(latestYear),
      prevYear: String(previousYear),
      note: `Ringkasan impor sektor pangan Tahun ${latestYear} dibandingkan Tahun ${previousYear}.`,
      highlight: null,
      prevHighlight: null,
      highlightType: "none",
      sourceName: data?.sourceName ?? "-"
    },
    {
      ...
      year: String(latestYear),
      prevYear: String(previousYear),
      note: `Ringkasan total perdagangan sektor pangan Tahun ${latestYear} dibandingkan Tahun ${previousYear}.`,
      highlight: null,
      prevHighlight: null,
      highlightType: "none",
      sourceName: data?.sourceName ?? "-"
  ...
  const mapTitle = `Peta Sebaran Sektor Pangan ${metricLabel}`;
  const sourceName = data?.sourceName ?? "-";
  const tableTitle = `Top Mitra ${metricLabel} Sektor Pangan`;
  const comparisonYearsLabel =
    previousYear !== latestYear
      ? `${previousYear}-${latestYear}`
      : String(latestYear);
  const mapSubtitle = loading
    ? "Sedang mengambil data peta sektor pangan..."
    : `Tahun ${comparisonYearsLabel} | Unit: ${data?.unit ?? "US$"}`;
  const tableSubtitle = loading
    ? "Sedang mengambil data top mitra sektor pangan..."
  ...
  async function handleDownloadMap() {
    try {
      ...
      await downloadComposedPng({
        imageUrl,
        filename: `Peta_Sebaran_Sektor_Pangan_${metricLabel.replace(/[^\w]+/g, "_")}`,
        title: mapTitle,
  ...
  const tableContent = (
    <TopMitraTable
      ...
      emptyMessage={`Data ${metricLabel.toLowerCase()} sektor pangan belum tersedia untuk filter aktif.`}
      onRegisterDownload={registerTableDownload}
      onSortColumnChange={handleSortColumnChange}
      downloadTitle={tableTitle}
      downloadFilename={`Top_Mitra_Sektor_Pangan_${metricLabel.replace(/[^\w]+/g, "_")}`}
