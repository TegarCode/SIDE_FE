...
export type PertahananMetricMode =
  | "total_export"
  | "total_import"
  | "total_trade";

type PertahananTradeHeroSectionProps = {
  metricMode: PertahananMetricMode;
  onMetricChange: (value: PertahananMetricMode) => void;
};

export function PertahananTradeHeroSection({
  metricMode,
  onMetricChange
}: PertahananTradeHeroSectionProps) {
  return (
    <section className="space-y-3">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div className="space-y-1">
          <h2 className="text-lg font-semibold tracking-tight text-slate-900">
            Perdagangan Indonesia Ke Global Sektor Pertahanan
          </h2>
          <p className="max-w-3xl text-sm text-slate-600">
            Ringkasan analitik perdagangan sektor pertahanan Indonesia ke global
