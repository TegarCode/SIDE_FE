...
import type { PertahananTradeFlowResult } from "@/type/komoditasUtama";
import { downloadComposedPng } from "@/utils/downloadComposedPng";
import type { PertahananMetricMode } from "./PertahananTradeHeroSection";

type PertahananTradeOverviewGridProps = {
  metricMode: PertahananMetricMode;
  data: PertahananTradeFlowResult | null | undefined;
  loading?: boolean;
};
...
function getMetricLabel(metricMode: PertahananMetricMode) {
...
export function PertahananTradeOverviewGrid({
  metricMode,
  data,
  loading = false
}: PertahananTradeOverviewGridProps) {
  ...
  const summaryCards: DiplomasiSummaryCardView[] = [
    {
      ...
      note: `Ringkasan ekspor sektor pertahanan Tahun ${latestYear} dibandingkan Tahun ${previousYear}.`,
      highlight: null,
      prevHighlight: null,
      highlightType: "none",
    ...
    },
    {
      ...
      year: String(latestYear),
      prevYear: String(previousYear),
      note: `Ringkasan impor sektor pertahanan Tahun ${latestYear} dibandingkan Tahun ${previousYear}.`,
      highlight: null,
      prevHighlight: null,
      highlightType: "none",
      sourceName: data?.sourceName ?? "-"
    },
    {
      ...
      year: String(latestYear),
      prevYear: String(previousYear),
      note: `Ringkasan total perdagangan sektor pertahanan Tahun ${latestYear} dibandingkan Tahun ${previousYear}.`,
      highlight: null,
      prevHighlight: null,
      highlightType: "none",
      sourceName: data?.sourceName ?? "-"
  ...
  const mapTitle = `Peta Sebaran Sektor Pertahanan ${metricLabel}`;
  const sourceName = data?.sourceName ?? "-";
  const tableTitle = `Top Mitra ${metricLabel} Sektor Pertahanan`;
  const comparisonYearsLabel =
    previousYear !== latestYear
      ? `${previousYear}-${latestYear}`
      : String(latestYear);
  const mapSubtitle = loading
    ? "Sedang mengambil data peta sektor pertahanan..."
    : `Tahun ${comparisonYearsLabel} | Unit: ${data?.unit ?? "US$"}`;
  const tableSubtitle = loading
    ? "Sedang mengambil data top mitra sektor pertahanan..."
  ...
  async function handleDownloadMap() {
    try {
      ...
      await downloadComposedPng({
        imageUrl,
        filename: `Peta_Sebaran_Sektor_Pertahanan_${metricLabel.replace(/[^\w]+/g, "_")}`,
        title: mapTitle,
  ...
  const tableContent = (
    <TopMitraTable
      ...
      emptyMessage={`Data ${metricLabel.toLowerCase()} sektor pertahanan belum tersedia untuk filter aktif.`}
      onRegisterDownload={registerTableDownload}
      onSortColumnChange={handleSortColumnChange}
      downloadTitle={tableTitle}
      downloadFilename={`Top_Mitra_Sektor_Pertahanan_${metricLabel.replace(/[^\w]+/g, "_")}`}
