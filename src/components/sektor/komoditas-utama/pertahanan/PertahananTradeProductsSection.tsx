...
import type { PertahananTradeProductsResult } from "@/type/komoditasUtama";
import type { PertahananMetricMode } from "./PertahananTradeHeroSection";

type PertahananTradeProductsSectionProps = {
  data: PertahananTradeProductsResult | null | undefined;
  loading: boolean;
...
function getMetricKey(metricMode: PertahananMetricMode) {
...
export function PertahananTradeProductsSection({
  data,
  loading,
  metricMode
}: PertahananTradeProductsSectionProps) {
  ...
  const cardSubtitle = loading
    ? "Sedang mengambil data produk sektor pertahanan..."
    : viewMode === "table"
  ...
  const titleNode = (
    <span className="inline-flex items-center gap-1.5">
      <span>{`Produk ${metricLabel} Sektor Pertahanan`}</span>
      <HoverInfoTooltip
        content={
  ...
  const tableContent = (
    <div className="space-y-2">
      <TopProdukTable
        raw={tableRaw}
        unitLabel={unitLabel}
        downloadTitle={`Produk ${metricLabel} Sektor Pertahanan`}
        downloadFilename={`Produk_Sektor_Pertahanan_${metricLabel.replace(/[^\w]+/g, "_")}`}
        ...
        emptyMessage={`Data produk ${metricLabel.toLowerCase()} sektor pertahanan belum tersedia.`}
      />
  ...
  const treemapBaseProps = {
    ...
    filename: `Treemap_Produk_Sektor_Pertahanan_${metricLabel.replace(/[^\w]+/g, "_")}`,
    exportTitle: `Treemap Produk ${metricLabel} Sektor Pertahanan`,
    exportSubtitle,
