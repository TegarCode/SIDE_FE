...
export type TikMetricMode = "total_export" | "total_import" | "total_trade";

type TikTradeHeroSectionProps = {
  metricMode: TikMetricMode;
  onMetricChange: (value: TikMetricMode) => void;
};

export function TikTradeHeroSection({
  metricMode,
  onMetricChange
}: TikTradeHeroSectionProps) {
  return (
    <section className="space-y-3">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div className="space-y-1">
          <h2 className="text-lg font-semibold tracking-tight text-slate-900">
            Perdagangan Indonesia Ke Global Sektor TIK
          </h2>
          <p className="max-w-3xl text-sm text-slate-600">
            Ringkasan analitik perdagangan sektor TIK Indonesia ke global
