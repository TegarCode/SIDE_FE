...
import type { TikTradeProductsResult } from "@/type/komoditasUtama";
import type { TikMetricMode } from "./TikTradeHeroSection";

type TikTradeProductsSectionProps = {
  data: TikTradeProductsResult | null | undefined;
  loading: boolean;
...
function getMetricKey(metricMode: TikMetricMode) {
...
export function TikTradeProductsSection({
  data,
  loading,
  metricMode
}: TikTradeProductsSectionProps) {
  ...
  const cardSubtitle = loading
    ? "Sedang mengambil data produk sektor TIK..."
    : viewMode === "table"
  ...
  const titleNode = (
    <span className="inline-flex items-center gap-1.5">
      <span>{`Produk ${metricLabel} Sektor TIK`}</span>
      <HoverInfoTooltip
        content={
          <div className="space-y-2">
            ...
            <p>
              ...
              memfokuskan distribusi produk pada tahun terbaru.
            </p>
          </div>
        }
      >
  ...
  const tableContent = (
    <div className="space-y-2">
      <TopProdukTable
        raw={tableRaw}
        unitLabel={unitLabel}
        downloadTitle={`Produk ${metricLabel} Sektor TIK`}
        downloadFilename={`Produk_Sektor_TIK_${metricLabel.replace(/[^\w]+/g, "_")}`}
        ...
        emptyMessage={`Data produk ${metricLabel.toLowerCase()} sektor TIK belum tersedia.`}
      />
  ...
  const treemapContent = (
    <div className="space-y-2">
      ...
      <TradeProductsTreemapChart
        ...
        filename={`Treemap_Produk_Sektor_TIK_${metricLabel.replace(/[^\w]+/g, "_")}`}
        exportTitle={`Treemap Produk ${metricLabel} Sektor TIK`}
