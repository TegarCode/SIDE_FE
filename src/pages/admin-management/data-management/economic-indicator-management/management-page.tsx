...
import { getAdminEconomicIndicatorDetailPath } from "@/constants/routes";
import { PERMISSIONS } from "@/constants/permissions";
import {
  useKinerjaEkonomiCurrentDataPage,
  useKinerjaEkonomiManagementPage
} from "@/hooks/admin-dashboard/useKinerjaEkonomiManagementPage";
import { fetchAdminKinerjaEkonomiDetail } from "@/service/admin-dashboard/kinerja-ekonomi";
...
export function AdminEconomicIndicatorManagementPage() {
  useDocumentTitle(`Manajemen Kinerja Ekonomi | ${APP_NAME}`);
  ...
  const [currentIndicatorFilter, setCurrentIndicatorFilter] = useState("");
  ...
  const {
    ...
    {
      countryCode: currentCountryFilter || undefined,
      indicatorId: currentIndicatorFilter || undefined,
  ...
  const usersQuery = useQuery({
    queryKey: ["admin-dashboard", "users", "economic-indicator-lookup"],
    queryFn: () =>
      fetchAdminUsers({
        page: 1,
        perPage: 9999,
  ...
  useEffect(() => {
    if (!canReadBatch) return;
    if (!batchesQuery.isError) return;
    toast({
      title: "Gagal memuat kinerja ekonomi",
      description: getApiErrorMessage(
        batchesQuery.error,
        "Daftar batch kinerja ekonomi tidak berhasil dimuat."
      ),
      tone: "error"
    });
  ...
  useEffect(() => {
    ...
    if (!currentDataQuery.isError) return;
    toast({
      title: "Gagal memuat data aktif",
      description: getApiErrorMessage(
        currentDataQuery.error,
        "Data aktif kinerja ekonomi tidak berhasil dimuat."
      ),
      tone: "error"
    });
    ...
    const intervalId = window.setInterval(() => {
      void Promise.all(
        trackedBatchIds.map(async (batchId) => {
          try {
            const result = await fetchAdminKinerjaEkonomiDetail(batchId);
  ...
  const handleCurrentPageChange = (nextPage: number) => {
    setCurrentPage(Math.min(Math.max(nextPage, 1), currentDataTotalPages));
  };

  const handleCurrentSearchSubmit = () => {
    setCurrentCountryFilter(currentCountryInput);
    setCurrentIndicatorFilter(currentIndicatorInput);
    setCurrentSourceFilter(currentSourceInput);
    setCurrentYearFilter(currentYearInput);
    setCurrentPage(1);
  };

  const handleCreate = async (payload: KinerjaEkonomiCreatePayload) => {
    try {
      ...
      toast({
        ...
        description: getApiErrorMessage(
          error,
          "Data kinerja ekonomi tidak berhasil disimpan."
        ),
        tone: "error"
      });
    }
  ...
  const handleManualSubmit = () => {
    const rows = manualRows.map(toPayloadRow).filter((row) => row.Kode_Alpha3);
    if (!rows.length) {
      toast({
        title: "Baris belum lengkap",
        description:
          "Isi minimal satu baris dengan Kode Alpha3 sebelum dikirim.",
        tone: "error"
      });
      return;
    }
  ...
  const summaryCards = [
    ...
    },
    {
      ...
      caption: "Batch yang sudah dipublikasikan.",
  ...
  return (
    <AdminManagementLayout
      title="Manajemen Kinerja Ekonomi"
      description="Kelola batch input dan import data kinerja ekonomi sampai publikasi."
    >
      <div className="mx-auto w-full max-w-7xl space-y-5">
        <PageTitle
          title="Manajemen Data Kinerja Ekonomi"
        ...
        {activeTab === "list" ? (
          <ListSection
            ...
            onSelectBatch={(batchId) =>
              navigate(getAdminEconomicIndicatorDetailPath(batchId))
            }
        ...
        {activeTab === "current" ? (
          <CurrentDataSection
            ...
            limit={currentLimit}
            countryInput={currentCountryInput}
            indicatorInput={currentIndicatorInput}
            ...
            onCountryInputChange={setCurrentCountryInput}
            onIndicatorInputChange={setCurrentIndicatorInput}
            ...
            onUpdateRow={async (row, payload) => {
              try {
                ...
                toast({
                  ...
                  description:
                    "Perubahan pada data aktif kinerja ekonomi telah disimpan.",
                  tone: "success"
                });
              } catch (error) {
                toast({
                  ...
                  description: getApiErrorMessage(
                    error,
                    "Data aktif kinerja ekonomi tidak berhasil disimpan."
                  ),
                  tone: "error"
                });
                throw error;
            ...
            onDeleteRow={async (row) => {
              try {
                ...
                toast({
                  ...
                  description:
                    "Baris data aktif kinerja ekonomi telah dihapus.",
                  tone: "success"
                });
              } catch (error) {
                toast({
                  ...
                  description: getApiErrorMessage(
                    error,
                    "Data aktif kinerja ekonomi tidak berhasil dihapus."
                  ),
                  tone: "error"
                });
                throw error;
            ...
            onDeleteRows={async (rows) => {
              try {
                ...
                toast({
                  title: "Data aktif berhasil dihapus",
                  description: `${rows.length} baris data aktif kinerja ekonomi telah dihapus.`,
                  tone: "success"
                });
              } catch (error) {
                toast({
