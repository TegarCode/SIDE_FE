...
import { getAdminInvestmentDetailPath } from "@/constants/routes";
import {
  useInvestmentCurrentDataPage,
  useInvestmentManagementPage
} from "@/hooks/admin-dashboard/useInvestmentManagementPage";
import { useDocumentTitle } from "@/hooks/useDocumentTitle";
import { fetchAdminInvestmentDetail } from "@/service/admin-dashboard/investment";
...
export function AdminInvestmentManagementPage() {
  useDocumentTitle(`Manajemen Investasi | ${APP_NAME}`);
  ...
  const [currentInvestmentTypeFilter, setCurrentInvestmentTypeFilter] =
    useState("");
  ...
  const [currentInvestmentTypeInput, setCurrentInvestmentTypeInput] =
    useState("");
  ...
  const {
  ...
  } = useInvestmentCurrentDataPage(
    {
      ...
      investmentType: currentInvestmentTypeFilter || undefined,
      page: currentPage,
  ...
  const usersQuery = useQuery({
    queryKey: ["admin-dashboard", "users", "investment-lookup"],
    queryFn: () =>
      fetchAdminUsers({
        page: 1,
        perPage: 9999,
  ...
  const investmentTypeOptions = useMemo(
    () =>
      (options?.investmentTypes ?? []).map((item) => ({
        value: item.value,
        label: item.label
      })),
  ...
  useEffect(() => {
    ...
    toast({
      ...
      description: getApiErrorMessage(
        batchesQuery.error,
        "Daftar batch investasi tidak berhasil dimuat."
      ),
      tone: "error"
    });
  ...
  useEffect(() => {
    ...
    toast({
      ...
      description: getApiErrorMessage(
        ...
        "Data aktif investasi tidak berhasil dimuat."
      ),
      tone: "error"
    });
    ...
    const intervalId = window.setInterval(() => {
      void Promise.all(
        trackedBatchIds.map(async (batchId) => {
          try {
            const result = await fetchAdminInvestmentDetail(batchId);
  ...
  const handleCurrentSearchSubmit = () => {
    ...
    setCurrentInvestmentTypeFilter(currentInvestmentTypeInput);
    setCurrentPage(1);
  };

  const handleCreate = async (payload: InvestmentCreatePayload) => {
    try {
      ...
      toast({
        ...
        description: getApiErrorMessage(
          error,
          "Data investasi tidak berhasil disimpan."
        ),
        tone: "error"
      });
    }
  ...
  const summaryCards = [
    ...
    },
    {
      ...
      caption: "Batch yang sudah dipublikasi ke `tbinvestment`.",
  ...
  return (
    <AdminManagementLayout
      ...
      description="Kelola batch input dan impor data investasi sampai publikasi."
    >
      <div className="mx-auto w-full max-w-7xl space-y-5">
        <PageTitle
          title="Manajemen Data Investasi"
        ...
        {activeTab === "list" ? (
          <ListSection
            ...
            onSelectBatch={(batchId) =>
              navigate(getAdminInvestmentDetailPath(batchId))
            }
        ...
        {activeTab === "current" ? (
          <CurrentDataSection
            ...
            investmentTypeInput={currentInvestmentTypeInput}
            sortBy={currentSortBy}
            sortDirection={currentSortDirection}
            ...
            statusOptions={investmentStatusOptions.filter(
              (item) => item.value !== ""
            )}
            ...
            onInvestmentTypeInputChange={setCurrentInvestmentTypeInput}
            onPageChange={(nextPage) =>
              setCurrentPage(
            ...
            onUpdateRow={async (row, payload) => {
              try {
                ...
                toast({
                  ...
                  description:
                    "Perubahan pada data aktif investasi telah disimpan.",
                  tone: "success"
                });
              } catch (error) {
                toast({
                  ...
                  description: getApiErrorMessage(
                    error,
                    "Data aktif investasi tidak berhasil disimpan."
                  ),
                  tone: "error"
                });
                throw error;
            ...
            onDeleteRow={async (row) => {
              try {
                ...
                toast({
                  ...
                  description: "Baris data aktif investasi telah dihapus.",
                  tone: "success"
                });
              } catch (error) {
                toast({
                  ...
                  description: getApiErrorMessage(
                    error,
                    "Data aktif investasi tidak berhasil dihapus."
                  ),
                  tone: "error"
                });
                throw error;
            ...
            onDeleteRows={async (rows) => {
              try {
                ...
                toast({
                  title: "Data aktif berhasil dihapus",
                  description: `${rows.length} baris data aktif investasi telah dihapus.`,
                  tone: "success"
                });
              } catch (error) {
                toast({
        ...
        {activeTab === "manual" ? (
          <ManualInputSection
            ...
            sectorOptions={sectorOptions}
            sourceOptions={sourceOptions}
            statusOptions={investmentStatusOptions.filter((item) => item.value)}
            monthOptions={monthOptions}
