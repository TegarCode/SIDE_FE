...
import { useTourismManagementPage } from "@/hooks/admin-dashboard/useTourismManagementPage";
...
export function AdminTourismDetailPage() {
  useDocumentTitle(`Detail Batch Pariwisata | ${APP_NAME}`);
  ...
  const [editingRow, setEditingRow] = useState<TourismRowRecord | null>(null);
  const [deletingRow, setDeletingRow] = useState<TourismRowRecord | null>(null);
  ...
  } = useTourismManagementPage({ page: 1, perPage: 1 }, batchId, {
    page: rowPage,
  ...
  const columns = [
    ...
    { key: "Provinsi_Tujuan", label: "Provinsi Tujuan", className: "min-w-32" },
  ...
  return (
    <AdminManagementLayout
      title="Detail Batch Pariwisata"
      description="Tinjau rincian batch data pariwisata, status validasi, dan daftar baris data yang masuk ke proses pengelolaan."
    >
      <div className="mx-auto w-full max-w-7xl space-y-5">
        <PageTitle
          title="Detail Batch Pariwisata"
          description="Halaman ini menampilkan ringkasan batch, status proses, waktu pemrosesan, serta baris data pariwisata yang dapat diperiksa, diubah, atau dihapus sesuai hak akses."
          actions={
        ...
        {detailQuery.isLoading ? (
          ...
          <EmptyStatePanel
            ...
            description="Detail batch pariwisata belum berhasil dimuat atau data tidak tersedia."
            compact
          />
        ) : (
        ...
        <EditRowModal
          ...
          statusOptions={tourismStatusOptions}
          ...
          onSubmit={async (row, payload) => {
            try {
              ...
              toast({
                ...
                description:
                  "Perubahan pada baris data pariwisata telah disimpan.",
                tone: "success"
              });
              setEditingRow(null);
            } catch (error) {
              toast({
                ...
                description: getApiErrorMessage(
                  error,
                  "Baris data pariwisata tidak berhasil disimpan."
                ),
                tone: "error"
              });
            }
        ...
        <ConfirmationModal
          ...
          title="Hapus Baris Data Pariwisata"
          subtitle="Konfirmasi penghapusan satu baris data dari batch pariwisata."
          ...
          onConfirm={async () => {
            ...
            try {
              ...
              toast({
                ...
                description: "Baris data pariwisata telah dihapus dari batch.",
                tone: "success"
              });
              setDeletingRow(null);
              setSelectedRowIds((current) => {
              ...
              toast({
                ...
                description: getApiErrorMessage(
                  error,
                  "Baris data pariwisata tidak berhasil dihapus."
                ),
                tone: "error"
              });
            }
        ...
        <ConfirmationModal
          ...
          subtitle="Konfirmasi penghapusan beberapa baris data dari batch pariwisata."
          ...
          onConfirm={async () => {
            ...
            try {
              ...
              toast({
                title: "Baris data berhasil dihapus",
                description: `${rowIds.length} baris data pariwisata telah dihapus dari batch.`,
                tone: "success"
              });
...
function BatchSummaryCards({ batch }: { batch: TourismBatchRecord }) {
  const cards: Array<{
    label: string;
    value: ReactNode;
    description: string;
  ...
  }> = [
    {
      ...
      description: "Asal input batch pariwisata",
