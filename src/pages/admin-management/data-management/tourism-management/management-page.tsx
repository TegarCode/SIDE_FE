...
import { getAdminTourismDetailPath } from "@/constants/routes";
import {
  useTourismCurrentDataPage,
  useTourismManagementPage
} from "@/hooks/admin-dashboard/useTourismManagementPage";
import { useDocumentTitle } from "@/hooks/useDocumentTitle";
import { fetchAdminTourismDetail } from "@/service/admin-dashboard/tourism";
...
export function AdminTourismManagementPage() {
  useDocumentTitle(`Manajemen Pariwisata | ${APP_NAME}`);
  ...
  const [currentTravelPurposeFilter, setCurrentTravelPurposeFilter] =
    useState("");
  const [currentOriginInput, setCurrentOriginInput] = useState("");
  ...
  const [currentTravelPurposeInput, setCurrentTravelPurposeInput] =
    useState("");
  ...
  const {
    ...
    {
      originCode: currentOriginFilter || undefined,
      destinationCode: currentDestinationFilter || undefined,
      sourceCode: currentSourceFilter || undefined,
      status: currentStatusFilter || undefined,
      year: currentYearFilter || undefined,
      month: currentMonthFilter || undefined,
      travelPurpose: currentTravelPurposeFilter || undefined,
      page: currentPage,
  ...
  const usersQuery = useQuery({
    queryKey: ["admin-dashboard", "users", "tourism-lookup"],
    queryFn: () =>
      fetchAdminUsers({
        page: 1,
        perPage: 9999,
  ...
  const tourismStatusOptions = useMemo(
    () => [
  ...
  const travelPurposeOptions = useMemo(
    () =>
      (options?.travelPurposes ?? []).map((item) => ({
        value: item.value,
        label: item.label
      })),
  ...
  useEffect(() => {
    ...
    toast({
      title: "Gagal memuat pariwisata",
      description: getApiErrorMessage(
        batchesQuery.error,
        "Daftar batch pariwisata tidak berhasil dimuat."
      ),
      tone: "error"
    });
  ...
  useEffect(() => {
    ...
    toast({
      ...
      description: getApiErrorMessage(
        ...
        "Data aktif pariwisata tidak berhasil dimuat."
      ),
      tone: "error"
    });
    ...
    const intervalId = window.setInterval(() => {
      void Promise.all(
        trackedBatchIds.map(async (batchId) => {
          try {
            const result = await fetchAdminTourismDetail(batchId);
  ...
  const handleCurrentSearchSubmit = () => {
    setCurrentOriginFilter(currentOriginInput);
    setCurrentDestinationFilter(currentDestinationInput);
    setCurrentSourceFilter(currentSourceInput);
    setCurrentStatusFilter(currentStatusInput);
    setCurrentYearFilter(currentYearInput);
    setCurrentMonthFilter(currentMonthInput);
    setCurrentTravelPurposeFilter(currentTravelPurposeInput);
    setCurrentPage(1);
  };

  const handleCreate = async (payload: TourismCreatePayload) => {
    try {
      ...
      toast({
        ...
        description: getApiErrorMessage(
          error,
          "Data pariwisata tidak berhasil disimpan."
        ),
        tone: "error"
      });
    }
  ...
  const handleManualSubmit = () => {
    const rows = manualRows
      .map(toPayloadRow)
      .filter((row) => row.Kode_Alpha3_Asal && row.Kode_Alpha3_Tujuan);

    if (!rows.length) {
      toast({
        title: "Baris belum lengkap",
        description:
          "Isi minimal satu baris dengan kode asal dan kode tujuan sebelum dikirim.",
        tone: "error"
      });
      return;
    }
  ...
  const summaryCards = [
    ...
    },
    {
      ...
      caption: "Batch yang sudah dipublikasi ke `tbtourism`.",
  ...
  return (
    <AdminManagementLayout
      ...
      description="Kelola batch input dan impor data pariwisata sampai publikasi."
    >
      <div className="mx-auto w-full max-w-7xl space-y-5">
        <PageTitle
          title="Manajemen Data Pariwisata"
        ...
        {activeTab === "list" ? (
          <ListSection
            ...
            onSelectBatch={(batchId) =>
              navigate(getAdminTourismDetailPath(batchId))
            }
        ...
        {activeTab === "current" ? (
          <CurrentDataSection
            ...
            limit={currentLimit}
            originInput={currentOriginInput}
            destinationInput={currentDestinationInput}
            sourceInput={currentSourceInput}
            statusInput={currentStatusInput}
            yearInput={currentYearInput}
            monthInput={currentMonthInput}
            travelPurposeInput={currentTravelPurposeInput}
            sortBy={currentSortBy}
            sortDirection={currentSortDirection}
            ...
            statusOptions={tourismStatusOptions.filter(
              (item) => item.value !== ""
            )}
            travelPurposeOptions={travelPurposeOptions}
            ...
            onOriginInputChange={setCurrentOriginInput}
            onDestinationInputChange={setCurrentDestinationInput}
            onSourceInputChange={setCurrentSourceInput}
            onStatusInputChange={setCurrentStatusInput}
            onYearInputChange={setCurrentYearInput}
            onMonthInputChange={setCurrentMonthInput}
            onTravelPurposeInputChange={setCurrentTravelPurposeInput}
            onPageChange={(nextPage) =>
              setCurrentPage(
            ...
            onUpdateRow={async (row, payload) => {
              try {
                ...
                toast({
                  ...
                  description:
                    "Perubahan pada data aktif pariwisata telah disimpan.",
                  tone: "success"
                });
              } catch (error) {
                toast({
                  ...
                  description: getApiErrorMessage(
                    error,
                    "Data aktif pariwisata tidak berhasil disimpan."
                  ),
                  tone: "error"
                });
                throw error;
            ...
            onDeleteRow={async (row) => {
              try {
                ...
                toast({
                  ...
                  description: "Baris data aktif pariwisata telah dihapus.",
                  tone: "success"
                });
              } catch (error) {
                toast({
                  ...
                  description: getApiErrorMessage(
                    error,
                    "Data aktif pariwisata tidak berhasil dihapus."
                  ),
                  tone: "error"
                });
                throw error;
            ...
            onDeleteRows={async (rows) => {
              try {
                ...
                toast({
                  title: "Data aktif berhasil dihapus",
                  description: `${rows.length} baris data aktif pariwisata telah dihapus.`,
                  tone: "success"
                });
              } catch (error) {
                toast({
        ...
        {activeTab === "manual" ? (
          <ManualInputSection
            ...
            countryOptions={countryOptions}
            sourceOptions={sourceOptions}
            statusOptions={tourismStatusOptions.filter((item) => item.value)}
