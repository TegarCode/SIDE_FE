...
import { MitraServiceSummarySection } from "@/components/mitra/jasa/MitraServiceSummarySection";
import { MitraServiceTopProductsSection } from "@/components/mitra/jasa/MitraServiceTopProductsSection";
import { MitraServiceTrendSection } from "@/components/mitra/jasa/MitraServiceTrendSection";
import { toTopMitraRaw } from "@/components/mitra/jasa/helpers";
...
import { useMitraMultiServiceQuery } from "@/hooks/mitra/useMitraMultiServiceQuery";
import { useMitraSingleServiceQuery } from "@/hooks/mitra/useMitraSingleServiceQuery";
import type { MitraFilterState } from "@/type/mitra";
import { downloadMitraServiceSummaryPdf } from "@/service/mitra";
...
const MITRA_JASA_TOUR_STEPS: GuidedTourStep[] = [
  {
    selector: "[data-mitra-jasa-tour='page-title']",
    title: "Pahami konteks halaman",
    description:
      "Halaman ini menampilkan ringkasan, tren, dan komposisi jasa untuk negara mitra."
  },
  {
    selector: "[data-mitra-jasa-tour='download-button']",
    title: "Unduh ringkasan PDF",
    description:
      "Gunakan tombol ini untuk mengunduh ringkasan PDF sesuai negara, filter rute, dan data jasa yang sedang ditampilkan."
  },
  {
    selector: "[data-mitra-jasa-tour='country-filter']",
    title: "Pilih negara mitra",
    description:
      "Gunakan filter negara untuk menentukan ringkasan dan tabel jasa yang ingin ditampilkan."
  },
  {
    selector: "[data-mitra-jasa-tour='summary-section']",
    title: "Lihat ringkasan dan tabel jasa",
    description:
      "Bagian ini menampilkan ringkasan jasa masuk/keluar dan tabel detail untuk negara yang dipilih."
  },
  {
    selector: "[data-mitra-jasa-tour='route-filter']",
    title: "Atur filter rute jasa",
    description:
      "Gunakan filter rute untuk memilih asal dan tujuan pada grafik tren tahunan jasa."
  },
  {
    selector: "[data-mitra-jasa-tour='trend-section']",
    title: "Baca tren tahunan jasa",
    description:
      "Bagian ini menunjukkan perkembangan jasa berdasarkan rute yang dipilih."
  },
  {
    selector: "[data-mitra-jasa-tour='top-products-section']",
    title: "Lihat top jasa",
    description:
      "Bagian ini membantu membaca kategori jasa utama yang paling dominan."
  }
];

export function MitraJasaPage() {
  useDocumentTitle(`Jasa Negara Mitra | ${APP_NAME}`);
  ...
  const query = useMitraSingleServiceQuery(filters.country);
  const multiQuery = useMitraMultiServiceQuery(
  ...
  const toastKey = React.useMemo(() => {
    ...
    return `mitra-jasa-single-${filters.country.toUpperCase()}`;
  }, [filters.country]);
  const multiToastKey = React.useMemo(() => {
    if (
      ...
      return null;
    return `mitra-jasa-multi-${routeFilters.origins.join("|")}-${routeFilters.destinations.join("|")}`;
  ...
  React.useEffect(() => {
    ...
    if (query.isFetching) {
      ...
      loadingToastIdRef.current = toast({
        title: "Sedang memuat data jasa",
        description: `Memuat data jasa untuk ${selectedCountryLabel}.`,
        tone: "loading",
        durationMs: null
      });
      return;
  ...
  React.useEffect(() => {
    ...
    toast({
      title: "Data jasa berhasil dimuat",
      description: `Data jasa untuk ${selectedCountryLabel} berhasil dimuat.`,
      tone: "success"
    });
  }, [
    query.data,
  ...
  React.useEffect(() => {
    ...
    toast({
      title: "Data jasa gagal dimuat",
      description:
        query.error instanceof Error
          ? query.error.message
          : "Terjadi kesalahan saat memuat data jasa.",
      tone: "error"
    });
  }, [query.error, toast]);
  ...
  React.useEffect(() => {
    ...
    if (multiQuery.isFetching) {
      ...
      multiLoadingToastIdRef.current = toast({
        title: "Sedang memuat grafik jasa",
        description: `Memuat tren tahunan jasa untuk ${routeSummaryLabel}.`,
        tone: "loading",
        durationMs: null
      });
      return;
  ...
  React.useEffect(() => {
    ...
    toast({
      title: "Grafik jasa berhasil dimuat",
      description: `Tren tahunan jasa untuk ${routeSummaryLabel} berhasil dimuat.`,
      tone: "success"
    });
  }, [
    multiQuery.data,
  ...
  React.useEffect(() => {
    ...
    toast({
      title: "Grafik jasa gagal dimuat",
      description:
        multiQuery.error instanceof Error
          ? multiQuery.error.message
          : "Terjadi kesalahan saat memuat grafik tren tahunan jasa.",
      tone: "error"
    });
  ...
  React.useEffect(() => {
    ...
    toast({
      ...
      description:
        "Data jasa mitra tidak dapat dimuat karena akses Anda belum valid.",
      tone: "warning",
      durationMs: 3200
    });
  ...
  const handleDownloadSummary = React.useCallback(async () => {
    ...
    const loadingId = toast({
      ...
      description: "Ringkasan sektor jasa sedang diproses...",
      tone: "loading",
      durationMs: null
    });
    ...
    try {
      const filenameBase = `Ringkasan_Jasa_${filters.country}_${routeFilters.origins.join("-")}_${routeFilters.destinations.join("-")}.pdf`;
      const result = await downloadMitraServiceSummaryPdf(
        {
      ...
      toast({
        ...
        description: "File ringkasan sektor jasa siap digunakan.",
        tone: "success",
        durationMs: 2200
      });
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-slate-900">
      <div className="space-y-6 px-4 py-6 lg:px-8">
        <div data-mitra-jasa-tour="page-title">
          <PageTitle
            title="Sektor Jasa"
            description="Data dan tren kerjasama sektor jasa negara/entitas mitra. Angka yang ditampilkan umumnya menggunakan tahun data terbaru yang tersedia."
            actions={
              <Button
                type="button"
                variant="primary"
                ...
                data-mitra-jasa-tour="download-button"
        ...
        <div data-mitra-jasa-tour="country-filter">
        ...
        <div className="space-y-6">
          {hasUnauthorizedError ? (
            <UnauthorizedAccessNotice
              title="Data jasa mitra memerlukan sesi login yang aktif"
              body="Permintaan ke layanan jasa mitra menerima respons 401. Masuk kembali lalu muat ulang halaman ini untuk melihat ringkasan, tabel, tren tahunan, dan top jasa."
            />
          ) : !filters.country ? (
            <FilterFallbackCard
              title="Negara / entitas mitra belum dipilih"
              body="Pilih satu negara mitra untuk menampilkan ringkasan dan tabel jasa."
            />
          ) : query.error ? (
            <FilterFallbackCard
              title="Data jasa negara mitra gagal dimuat"
              body={
                query.error instanceof Error
                  ? query.error.message
                  : "Terjadi kesalahan saat mengambil data jasa."
              }
            />
          ) : !query.isLoading && !query.data ? (
            <FilterFallbackCard
              title="Data jasa negara mitra belum tersedia"
              body={`Data jasa untuk ${selectedCountryLabel} belum tersedia.`}
            />
          ) : (
            <div className="space-y-6" data-mitra-jasa-tour="summary-section">
              <MitraServiceSummarySection
                data={query.data}
              ...
              <section className="space-y-3">
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-500">
                    Daftar negara/entitas untuk jasa masuk dan jasa keluar pada
                ...
                <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
                  ...
                  <MitraInvestmentTableCard
                    ...
                    prevYear={query.data?.prevYear ?? null}
                    loading={query.isLoading && !query.data}
                    emptyMessage={`Data jasa keluar dari ${selectedCountryLabel} belum tersedia.`}
                  />
                </div>
              </section>
            </div>
          ...
          <div data-mitra-jasa-tour="route-filter">
            <MitraInvestmentRouteFiltersPanel
              value={routeFilters}
              loading={masterQuery.isLoading}
              requestLoading={masterQuery.isFetching || multiQuery.isFetching}
              title="Filter Rute Jasa"
              description="Pilih asal dan tujuan untuk menampilkan grafik tren tahunan dan top jasa masuk & keluar. Klik header untuk membuka atau menutup filter."
          ...
          {hasUnauthorizedError ? null : multiQuery.error ? (
            <FilterFallbackCard
              title="Grafik tren tahunan jasa gagal dimuat"
              body={
                multiQuery.error instanceof Error
                  ? multiQuery.error.message
                  : "Terjadi kesalahan saat mengambil grafik tren tahunan jasa."
              }
            />
          ) : (
            <div data-mitra-jasa-tour="trend-section">
              <MitraServiceTrendSection
          ...
          {hasUnauthorizedError || multiQuery.error ? null : (
            <div data-mitra-jasa-tour="top-products-section">
              <MitraServiceTopProductsSection
                data={multiQuery.data}
                loading={multiQuery.isLoading && !multiQuery.data}
              />
            </div>
      ...
      <GuidedTour
        steps={MITRA_JASA_TOUR_STEPS}
        storageKey="side-mitra-jasa-tour-completed"
