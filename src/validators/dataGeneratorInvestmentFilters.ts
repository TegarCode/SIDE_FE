...
export type DataGeneratorInvestmentFilterValueLike = {
  origins: string[];
...
export type DataGeneratorInvestmentFilterErrors = Partial<{
...
export const dataGeneratorInvestmentFiltersSchema = yup
  .object({
    ...
    investmentType: yup
      ...
      .nullable()
      .defined()
      .required("Tipe investasi wajib dipilih."),
    source: yup
      .string()
      .nullable()
      .defined()
...
export function validateDataGeneratorInvestmentFilters(
  value: DataGeneratorInvestmentFilterValueLike
): DataGeneratorInvestmentFilterErrors {
  const errors: DataGeneratorInvestmentFilterErrors = {};

  try {
    dataGeneratorInvestmentFiltersSchema.validateSync(value, {
      abortEarly: false
    });
  } catch (error) {
    if (error instanceof yup.ValidationError) {
      for (const issue of error.inner) {
        ...
        errors[issue.path as keyof DataGeneratorInvestmentFilterErrors] =
          issue.message;
      }
    }
  }
