...
export type DataGeneratorServiceFilterValueLike = {
  origins: string[];
...
export type DataGeneratorServiceFilterErrors = Partial<{
...
export const dataGeneratorServiceFiltersSchema = yup
  .object({
...
export function validateDataGeneratorServiceFilters(
  value: DataGeneratorServiceFilterValueLike
): DataGeneratorServiceFilterErrors {
  const errors: DataGeneratorServiceFilterErrors = {};

  try {
    dataGeneratorServiceFiltersSchema.validateSync(value, {
      abortEarly: false
    });
  } catch (error) {
    if (error instanceof yup.ValidationError) {
      for (const issue of error.inner) {
        ...
        errors[issue.path as keyof DataGeneratorServiceFilterErrors] =
          issue.message;
      }
    }
  }
